...
CREATE TABLE sbtest%d(
  ...
  %s (id)
   ...
   if sysbench.opt.create_secondary then
	...
	CREATE TABLE sbtest%d(
	  ...
	  %s (id),
	      ...
	      table_num, id_def, id_index_def, table_num, engine_def, extra_table_options)
   else
	   query = string.format([[
	CREATE TABLE sbtest%d(
	  id %s,
   ...
   print("sleeping 40s to wait for schema sync");
   sleep(40)
   ...
   for i = 1, sysbench.opt.table_size do
      ...
      if (sysbench.opt.auto_inc) then
         query = string.format("(%d, '%s', '%s')",
                               sysbench.rand.default(1, sysbench.opt.table_size), c_val,
                               pad_val)
      else
         query = string.format("(%d, %d, '%s', '%s')",
                               i, sysbench.rand.default(1, sysbench.opt.table_size), c_val,
                               pad_val)
      end

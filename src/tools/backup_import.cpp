...
std::string _gen_insert_sql(const std::string& database_name, const std::string& table_name, 
    ...
    std::string sql = "INSERT INTO " + database_name + "." + table_name;
    std::string names;
    std::string values;
...
bool BackUpImport::need_to_trigger(int64_t date, int64_t hour, int64_t interval_days, int64_t& now_date) {
...
int BackUpImport::gen_backup_task() {
    ...
    while (result_set.next()) {
        ...
        if (need_to_trigger(date, hour, interval_days, now_date) == false) {
            continue;
        }
...
int BackUpImport::reset_legacy_task() {
    ...
    for (auto& id : ids) {
        std::map<std::string, std::string> set_map;
        std::map<std::string, std::string> where_map;
        set_map["status"]   = "'failed!!! baikalImporter maybe exit!'";
...
int BackUpImport::fetch_new_task() {
    ...
    while (result_set.next()) {
        ...
        int ret = bp.run_backup(table_ids);
        if (ret == 0) {

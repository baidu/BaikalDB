...
DEFINE_bool(remote_compaction_send_no_op, false, "default(false)");
...
int Region::init(bool new_region, int32_t snapshot_times) {
    ...
    if (new_region) {
        ...
        if (boost::filesystem::exists(snapshot_path)) {
            ...
            RegionControl::clear_all_infos_for_region(_region_id, _region_info.table_id());
        }
        // 被addpeer的node不需要init meta
        // on_snapshot_load时会ingest meta sst
        if (_region_info.peers_size() > 0) {
            need_init_column = true;
        ...
        for (int64_t index_id : table_info.indices) {
            ...
            switch (info.type) {
                case pb::I_FULLTEXT: 
                    ...
                    break;
                case pb::I_VECTOR:
                    _vector_index_map[index_id] = new VectorIndex();
                    if (_vector_index_map[index_id]->init(_region_info, _region_id, index_id, table_info.id) != 0) {
                        DB_FATAL("vector_index init fail when init reigon, region_id: %ld", _region_id);
                        return -1;
                    }
                    break;
                default:
    ...
    bool is_cold = false;
    if (_factory->is_olap_table(get_table_id(), get_partition_id(), &is_cold)) {
        // 判断是olap表，在启动时就设置为true
        _storage_compute_separate = true;
    }
    ...
    _txn_pool.init(_region_id, use_ttl(), use_normal_ttl(), _online_ttl_base_expire_time_us);
    ...
    if (need_init_column) {
        // add peer的region在on_snapshot_load时初始化
        if (_column_mgr.init()) {
            DB_COLUMN_FATAL("region_%ld fail to init column file", _region_id);
            return -1;
        }
    }

    std::string ttl_field_name = _ttl_field == nullptr ? "null" : _ttl_field->short_name;
    DB_WARNING("region_id: %ld init success, region_info:%s, time_cost:%ld ,table_id: %ld, "
               "ttl_type: %d, ttl_field: %s, ttl_duration_s: %ld, online_ttl_expire_time_us: %ld, %s",
                _region_id, _resource->region_info.ShortDebugString().c_str(), time_cost.get_time(),
                get_table_id(), static_cast<int32_t>(_ttl_type), ttl_field_name.c_str(), ttl_info.ttl_duration_s,
...
int Region::validate_version(const pb::StoreReq* request, pb::StoreRes* response, bool& is_valid_version) {
    if (request->region_version() < get_version()) {
        ...
        // MPP场景，通知上游ER处理
        if (request->plan().nodes().size() > 0 && request->plan().nodes(0).node_type() == pb::EXCHANGE_SENDER_NODE) {
            if (mpp_send_version_old(request->plan().nodes(0).derive_node().exchange_sender_node(), response->is_merge(), 
                                    _region_id, response->regions()) != 0) {
                DB_WARNING("Fail to mpp_send_version_old, %ld", _region_id);
                return -1;
            }
        }
...
int Region::apply_partial_rollback(google::protobuf::RpcController* controller,
            ...
            pb::StoreRes* response) {
    brpc::Controller* cntl = (brpc::Controller*)controller;
    pb::StoreReq raft_req;
    raft_req.set_op_type(pb::OP_PARTIAL_ROLLBACK);
    raft_req.set_region_id(_region_id);
    raft_req.set_region_version(get_version());
    pb::TransactionInfo txn_info = request->txn_infos(0);
    ...
    c->op_type = pb::OP_PARTIAL_ROLLBACK;
...
void Region::exec_in_txn_query(google::protobuf::RpcController* controller,
    ...
    if (FLAGS_apply_partial_rollback && txn != nullptr
        ...
        if (need_rollback) {
            if (apply_partial_rollback(controller, txn, request, response) != 0 || response->errcode() != pb::SUCCESS) {
    ...
    switch (op_type) {
        ...
        case pb::OP_ROLLBACK:
        case pb::OP_COMMIT: {
            if (_split_param.split_slow_down && (txn == nullptr || !txn->has_write())) {
                // 当事务已经执行了更改(持锁), 后续seq_id不再等待, 尽快执行完释放锁, 避免事务和分裂相互影响恶化
                // 但是大事务场景可能导致分裂变慢
                DB_WARNING("region is spliting, slow down time:%ld, region_id: %ld, txn_id: %lu:%d log_id:%lu remote_side: %s",
                            _split_param.split_slow_down_cost, _region_id, txn_id, seq_id, log_id, remote_side);
                bthread_usleep(_split_param.split_slow_down_cost);
...
void Region::exec_out_txn_query(google::protobuf::RpcController* controller,
    ...
    switch (op_type) {
        ...
        case pb::OP_ROLLUP_REGION_INIT:
        case pb::OP_ROLLUP_REGION_FINISH:
        case pb::OP_ROLLUP_REGION_FAILED:
        case pb::OP_SELECT_FOR_UPDATE: {
            if (_split_param.split_slow_down) {
...
void Region::exec_kv_out_txn(const pb::StoreReq* request, 
    ...
    StateOption option;
    option.store_compute_separate = true;
    option.is_binlog_region = _is_binlog_region;
    ret = state.init(*request, request->plan(), request->tuples(), &_txn_pool, option);
    if (ret < 0) {
        response->set_errcode(pb::EXEC_FAIL);
    ...
    // 如果是冷数据要读冷
    Transaction::TxnOptions txn_opts;
    if (_olap_state.load() >= pb::OLAP_FLUSHED && _rocksdb->has_init_cold_rocksdb()) {
        DB_WARNING("region_id: %ld use cold db", _region_id);
        txn_opts.use_cold_db = true;
    }
    state.create_txn_if_null(txn_opts);
    ...
    ret = ExecNode::create_tree(request->plan(), &root, CreateExecOptions());
    if (ret < 0) {
    ...
    if (request->op_type() == pb::OP_SELECT_FOR_UPDATE) {
        for (auto& tuple : state.tuple_descs()) {
            if (tuple.has_tuple_id()) {
                response->add_tuple_ids(tuple.tuple_id());
            }
        }
        int rows = select_normal(state, root, *request, *response);
        if (rows < 0) {
            root->close(&state);
            ExecNode::destroy_tree(root);
            response->set_errcode(pb::EXEC_FAIL);
    ...
    //等待所有raft执行完成
    state.txn_cond.wait();
    if (state.err_code != pb::SUCCESS) {
        response->set_errcode(state.err_code);
        response->set_errmsg(state.error_msg.str());
        DB_FATAL("_disable_write_cond wait timeout, log_id:%lu ret:%d, region_id: %ld", state.log_id(), ret, _region_id);
...
void Region::async_apply_log_entry(google::protobuf::RpcController* controller,
    ...
    for (; success_applied_cnt < request->request_lens_size(); ++success_applied_cnt) {
        if (store_responses[success_applied_cnt].errcode() != pb::SUCCESS
                && store_responses[success_applied_cnt].mysql_errcode() != ER_DUP_ENTRY) {
            // 会忽略 EXEC_FAIL + ER_DUP_ENTRY的执行失败
...
void Region::query(google::protobuf::RpcController* controller,
    ...
    if (!is_leader() && !request->extra_req().no_raft_log()) {
        ...
        if (request->extra_req().use_read_idx()) {
            ...
            // follower read
            TimeCost wait_time;
            SmartFollowerReadCond c = std::make_shared<FollowerReadCond>(is_learner(), log_id);
            ...
            int64_t read_index_total_cost = wait_time.get_time();
            Store::get_instance()->follow_read_wait_time << read_index_total_cost;
            if (read_index_total_cost > FLAGS_print_time_us) {
                DB_WARNING("region_id: %ld, log_id: %lu, read_index_total_cost: %ld, ask_rdx_time: %ld, wait_rdx_time: %ld, read_index: %ld", 
                    _region_id, log_id,
                    read_index_total_cost, c->ask_leader_read_index_cost, c->wait_read_index_cost, c->read_index);
            }
            if (ret != 0) {
                // 10s超时失败
                response->set_errcode(is_learner() ? pb::LEARNER_NOT_READY : pb::NOT_LEADER);
    ...
    switch (request->op_type()) {
        ...
        case pb::OP_ROLLUP_REGION_FAILED: {
            ...
            if (is_cold_data_op_type(op_type) && _olap_state.load() != pb::OLAP_ACTIVE) {
                ...
                DB_WARNING("immutable olap not support write, op_type:%s region_id: %ld, log_id:%lu",
                        pb::OpType_Name(op_type).c_str(), _region_id, log_id);
                return;
            }
            // 在rollup做ddl的时候只能insert merge into
            if (_rollup_region_init_index != -1 
                && is_unsupport_rollup_ddl_type(request)) {
                response->set_errcode(pb::UNSUPPORT_REQ_TYPE);
                response->set_errmsg("doing rollup ddl not support exclude insert merge into");
                DB_WARNING("doing rollup ddl not support exclude insert merge int, op_type:%s region_id: %ld, log_id:%lu",
                        pb::OpType_Name(op_type).c_str(), _region_id, log_id);
            }
            if (txn_id == 0 || op_type == pb::OP_TRUNCATE_TABLE) {
                exec_out_txn_query(controller, request, response, done_guard.release());
            } else {
...
void Region::dml_2pc(const pb::StoreReq& request, 
    ...
    StateOption option;
    option.store_compute_separate = is_separate;
    option.is_binlog_region = _is_binlog_region;
    ret = state.init(request, plan, tuples, &_txn_pool, option);
    if (ret < 0) {
        response.set_errcode(pb::EXEC_FAIL);
        response.set_errmsg("RuntimeState init fail");
    ...
    ExecNode* root = nullptr;
    ret = ExecNode::create_tree(plan, &root, CreateExecOptions());
    if (ret < 0) {
        ExecNode::destroy_tree(root);
        response.set_errcode(pb::EXEC_FAIL);
        response.set_errmsg("create plan fail");
...
void Region::dml_1pc(const pb::StoreReq& request, pb::OpType op_type,
    ...
    ON_SCOPE_EXIT([op_type]() {
            if (FLAGS_open_service_write_concurrency && (op_type == pb::OP_INSERT ||
                op_type == pb::OP_UPDATE ||
                op_type == pb::OP_DELETE)) {
                Concurrency::get_instance()->service_write_concurrency.decrease_signal();
            }
        });
    ...
    if (done == nullptr) {
        auto res = txn->commit(_region_id, applied_index);
        if (res.ok()) {
            commit_succ = true;
...
void Region::kv_apply_raft(RuntimeState* state, SmartTransaction txn) {
    ...
    int ret = _disable_write_cond.timed_wait(disable_write_wait);
    if (ret != 0) {
        state->err_code = pb::DISABLE_WRITE_TIMEOUT;
...
int Region::select(const pb::StoreReq& request, pb::StoreRes& response) {
    ...
    ON_SCOPE_EXIT([&]() {
        if (FLAGS_open_new_sign_read_concurrency && is_new_sign) {
            Concurrency::get_instance()->new_sign_read_concurrency.decrease_signal();
        }
        response.mutable_extra_res()->set_wait_cost(wait_cost);
...
int Region::select_vectorized(RuntimeState& state, ExecNode* root, const pb::StoreReq& request, pb::StoreRes& response) {
    // arrow不适合处理扫描量<500的请求, cpu耗费高, 故而第一个rowBatch走行存
    // 如果一个batch就执行完,就走行式执行, 否则走列式执行. 需要将第一个rowbatch结果merge到arrow
    bool eos = false;
    int rows = 0;
    int ret = 0;
    ...
    ExchangeSenderNode* exchange_sender_node = 
        static_cast<ExchangeSenderNode*>(root->get_node(pb::EXCHANGE_SENDER_NODE));
    // 有agg才能走parallel pipeline
    ExecNode* agg_node = root->get_node(pb::AGG_NODE);
    // 开启pipeline并发需要检查所有的bthread local, 尤其是scannode, 因为从第一个batch走行之后会切换bthread继续进行scan
    state.vectorlized_parallel_execution = FLAGS_vectorlized_parallel_execution && (agg_node != nullptr);
    if (request.execute_type() == pb::EXEC_ARROW_ACERO_PREFER_RETURN_ARROW_DATA) {
        state.force_vectorize = true;
    }

    int mpp_ret = -1;
    std::shared_ptr<arrow::RecordBatch> mpp_record_batch;
    ScopeGuard scope_guard([&exchange_sender_node, &mpp_record_batch, &mpp_ret] () {
        if (exchange_sender_node == nullptr) {
            return;
        }
        if (mpp_ret != 0) {
            exchange_sender_node->send_exec_fail();
        }
    });
    if (exchange_sender_node == nullptr) {
        while (!eos) {
            ...
            if (ret < 0) {
                DB_FATAL("arrow execute fail: plan get_next fail, region_id: %ld", _region_id);
                return -1;
            }
            // 第一个batch直接走完的, eos=true, 改走pb::EXEC_ROW
            // 第一个batch没走完的, eos=false, break列式执行
            if (root->node_exec_type() == pb::EXEC_ARROW_ACERO) {
                break;
            }
        ...
        if (root->node_exec_type() == pb::EXEC_ROW) {
            // 行式执行结束,直接返回行式结果
            return rows;
        }
    }
    ...
    if (ret != 0) {
        DB_FATAL("arrow execute fail: build arrow acero plan fail, region: %ld, log_id: %lu", _region_id, request.log_id());
        return -1;
    }
    // execute
    ...
    if (!final_table.ok()) {
        if (!state.is_timeout()) {
            DB_FATAL("arrow execute fail: region_id: %ld, logid: %lu, status: %s", 
                    _region_id, request.log_id(), final_table.status().ToString().c_str());
        }
        return -1;
    }
    int64_t runtime = t.get_time();
    t.reset();
    ...
    if (table->num_rows() == 0) {
        mpp_ret = 0;
        if (exchange_sender_node != nullptr && runtime > FLAGS_print_time_us) {
            DB_NOTICE("mpp request, region_id: %ld, time_cost: %ld, log_id: %lu, sign: %lu, rows: %ld, scan_rows: %ld, repartition_cost: %ld",
                      _region_id, runtime, request.log_id(), request.sql_sign(),
                      exchange_sender_node->get_repartition_rows() , 
                      state.num_scan_rows(),
                      exchange_sender_node->get_repartition_cost());
        }
        return 0;
    }
    if (limit > 0) {
    ...
    std::shared_ptr<arrow::RecordBatch> record_batch = *record_batch_result;
    ...
    arrow::Result<std::shared_ptr<arrow::Buffer>> data_ret = arrow::ipc::SerializeRecordBatch(*record_batch, arrow::ipc::IpcWriteOptions::Defaults());
    if (!data_ret.ok()) {
        DB_FATAL("arrow execute fail: serializeToString fail, region_id: %ld, logid: %lu, status: %s", 
                _region_id, request.log_id(), data_ret.status().ToString().c_str());
        return -1;
    } 
    response.mutable_extra_res()->set_vectorized_schema((*schema_ret)->data(), (*schema_ret)->size());
    response.mutable_extra_res()->set_vectorized_rows((*data_ret)->data(), (*data_ret)->size());
    //DB_WARNING("record_batch row: %d, runtime: %ld, combine_time: %ld, serializetime: %ld", rows, runtime, combine_time, t.get_time());
    //DB_WARNING("region_id: %ld, schema: %s, result table: %s", _region_id, record_batch->schema()->ToString().c_str(), record_batch->ToString().c_str());
    mpp_ret = 0;
    return rows;
}
...
int Region::select(const pb::StoreReq& request, 
    ...
    SmartState state_ptr = std::make_shared<RuntimeState>();
    RuntimeState& state = *state_ptr;
    state.execute_type = request.execute_type();
    state.set_resource(get_resource());
    StateOption option;
    option.is_binlog_region = _is_binlog_region;
    ...
    if (validate_version(&request, &response, is_valid_version) != 0) {
        response.set_errcode(pb::INTERNAL_ERROR);
        response.set_errmsg("Fail to validate_version");
        DB_WARNING("Fail to validate_version, region_id: %ld", _region_id);
        return -1;
    }
    if (is_valid_version == false) {
    ...
    if (txn != nullptr) {
        ...
        txn = state.create_txn_if_null(txn_opts);
    }
    ScopeGuard auto_rollback([&]() {
        if (is_new_txn) {
            txn->rollback();
    ...
    options.use_column_storage = txn_info.txn_id() == 0 ? use_column_storage(plan, table_ptr) : false;
    ret = ExecNode::create_tree(plan, &root, options);
    if (ret < 0) {
        ExecNode::destroy_tree(root);
        response.set_errcode(pb::EXEC_FAIL);
        response.set_errmsg("create plan fail");
    ...
    state.force_vectorize = FLAGS_only_use_arrow;
    if (root->node_type() == pb::EXCHANGE_SENDER_NODE) {
        state.force_vectorize = true;
    }
    ret = root->open(&state);
    if (ret < 0) {
    ...
    if (state.execute_type == pb::EXEC_ROW) {
        ...
        if (request.has_analyze_info()) {
            ...
            rows = select_normal(state, root, request, response);
        }
    } else {
        rows = select_vectorized(state, root, request, response);
    }
    response.set_execute_type(root->node_exec_type());
    if (rows < 0) {
        root->close(&state);
        ExecNode::destroy_tree(root);
        response.set_errcode(pb::EXEC_FAIL);
...
int Region::select_normal(RuntimeState& state, ExecNode* root, const pb::StoreReq& request, pb::StoreRes& response) {
    bool eos = false;
    int rows = 0;
    int ret = 0;
    MemRowDescriptor* mem_row_desc = state.mem_row_desc();
    TimeCost t;
    int64_t total_packet_time = 0;
    while (!eos) {
        RowBatch batch;
        batch.set_capacity(state.row_batch_capacity());
        ret = root->get_next(&state, &batch, &eos);
        ...
        total_packet_time += t.get_time();
    }
    if (total_packet_time > FLAGS_print_time_us) {
        DB_WARNING("region_id: %ld, log_id: %lu, total_packet_time: %ld", _region_id, request.log_id(), total_packet_time);
    }
    return rows;
}

inline int statistics_type_bitmap(const pb::AnalyzeInfo& analyze_info) {
    // 返回bitmap
    // 低零位: hist
    // 低一位: cms
    // 低二位: hll
    int bitmap = 0;
    for (const auto& request_st: analyze_info.statistics_types()) {
        bitmap |= (1 << request_st);
    }
    return bitmap;
}
...
int Region::select_sample(RuntimeState& state, ExecNode* root, const pb::AnalyzeInfo& analyze_info, pb::StoreRes& response) {
    ...
    if (analyze_info.sample_rows() >= analyze_info.table_rows()) {
        ...
        sample_cnt = analyze_info.sample_rows() * 1.0 / std::max(analyze_info.table_rows(), (int64_t)1) * _num_table_lines.load();
    }
    if (sample_cnt < 1) {
        sample_cnt = 1;
    }
    ...
    int type_bitmap = statistics_type_bitmap(analyze_info);
    bool need_cmsketch = type_bitmap & (1 << pb::StatisticType::ST_CMSKETCH);
    bool need_histogram = type_bitmap & (1<< pb::StatisticType::ST_HISTOGRAM);
    bool need_hyperloglog = type_bitmap & (1 << pb::StatisticType::ST_HYPERLOGLOG);
    bool is_clear = type_bitmap & (1 << pb::StatisticType::ST_CLEAR);
    if (is_clear) {
        return 0;
    }
    
    std::shared_ptr<CMsketch> cmsketch_ptr;
    std::shared_ptr<HyperLogLog> hll_ptr;
    if (need_cmsketch) {
        cmsketch_ptr = std::make_shared<CMsketch>(analyze_info.depth(), analyze_info.width());
    }
    if (need_hyperloglog) {
        hll_ptr = std::make_shared<HyperLogLog>();
    }
    ...
    while (!eos) {
        ...
        for (batch.reset(); !batch.is_traverse_over(); batch.next()) {
            for (auto& slot : tuple_desc->slots()) {
                ...
                uint64_t hash_value = value.hash();
                if (need_cmsketch) {
                    cmsketch_ptr->set_value(slot.field_id(), hash_value);
                }
                if (need_hyperloglog) {
                    hll_ptr->add_value(slot.field_id(), hash_value);
                }
            }
            count++;
            if (need_histogram) {
    ...
    if (need_cmsketch) {
        ...
        cmsketch_ptr->to_proto(cmsketch_pb);
    }
    if (need_hyperloglog) {
        pb::HyperLogLog* hll_pb = response.mutable_hll();
        hll_ptr->to_proto(hll_pb, true);
    }
    return rows;
}

void Region::construct_peers_status(pb::LeaderHeartBeat* leader_heart, bool& need_report) {
    ...
    for (auto iter : status.stable_followers) {
        ...
        peer_info->set_peer_status(pb::STATUS_NORMAL);
        peer_status_str += peer_info->peer_id() + ",";
    }
    peer_status_str += std::to_string(_version) + ",";
    peer_status_str += std::to_string(_region_control.get_status());
    ...
    if (_cache_peer_status != peer_status_str) {
        need_report = true;
    }
    if (!need_report) {
        leader_heart->clear_peers_status();
    }
    std::lock_guard<std::mutex> lock_region(_region_lock);
    _cache_peer_status = peer_status_str;
}

void Region::construct_heart_beat_request(pb::StoreHeartBeatRequest& request, bool need_peer_balance, bool meta_need_report) {
    ...
    if ((need_peer_balance || is_merged()) 
        ...
        bool has_exist_leader = false;
        if (get_leader().ip != butil::IP_ANY) {
            has_exist_leader = true;
        }
        add_peer_info(request, copy_region_info, _applied_index, is_learner(), has_exist_leader);
    }
    //添加leader的心跳信息，同时更新状态
    std::vector<braft::PeerId> peers;
    bool need_report = FLAGS_report_all_to_meta || meta_need_report || need_peer_balance || request.need_leader_balance();
    // 只有完全正常才不上报
    if (is_merged() || is_learner() || _is_binlog_region || _version == 0 || _region_control.get_status() != pb::IDLE) {
        need_report = true;
    }
    if (is_leader() && _node.list_peers(&peers).ok()) {
        ...
        if (table_ptr == nullptr) {
            ...
            // 前缀负载均衡需要上报
            if (table_ptr->schema_conf.pk_prefix_balance() > 0) {
                need_report = true;
            }
        }
        ...
        if (peers.size() != leader_region->replica_num()) {
            need_report = true;
        }
        ...
        construct_peers_status(leader_heart, need_report);
        ...
        if (need_report) {
            ...
            leader_region->clear_leader();
            leader_region->clear_start_key();
            leader_region->clear_end_key();
            leader_heart->set_simple(true);
        }
    } else {
        // 没leader后都需要重新上报
        std::lock_guard<std::mutex> lock_region(_region_lock);
...
void Region::do_apply(int64_t term, int64_t index, const pb::StoreReq& request, braft::Closure* done) {
    ...
    switch (op_type) {
        ...
        case pb::OP_OLAP_INFO: {
            apply_olap_info(request, done);
            break;
        }
        case pb::OP_OLAP_INDEX_INFO: {
            apply_olap_index_info(request, done);
            break;
        }
        case pb::OP_COLUMN_INFO: {
            apply_column_info(request, done);
            break;
        }
        //kv操作,存储计算分离时使用
        case pb::OP_KV_BATCH: {
            ...
            _data_index = _applied_index;
            if (_rollup_region_init_index != -1 && !request.is_rollup_base() && request.kv_ops_size() > 0) {
                // 有缓存且不是rollup_base, 暂时不apply
                break;
            }

            if (request.is_rollup_base() && _rollup_region_init_index == -1) {
                // 做rollup base的时候 _rollup_region_init_index == -1 说明环境被改变, 直接报错
                if (done != nullptr) {
                    ((DMLClosure*)done)->response->set_errcode(pb::EXEC_FAIL);
                    ((DMLClosure*)done)->response->set_mysql_errcode(ER_INDEX_CORRUPT);
                    ((DMLClosure*)done)->applied_index = _applied_index;
                    ((DMLClosure*)done)->response->set_errmsg("rollup base work is wrong");
                    DB_WARNING("op_type=%s, region_id: %ld, rollup base work is wrong",
                        pb::OpType_Name(request.op_type()).c_str(), _region_id);
                }
                break;
            }
            ...
            if (txn_id == 0) {
                apply_kv_out_txn(request, done, _applied_index, term, false);
            } else {
        ...
        case pb::OP_COMMIT:
        case pb::OP_ROLLBACK:
        case pb::OP_PARTIAL_ROLLBACK: {
            _data_index = _applied_index;
            apply_txn_request(request, done, _applied_index, term);
            if (FLAGS_enable_compute_storage_decoupled_mode && op_type == pb::OP_COMMIT && done != nullptr) {
                if (((DMLClosure*)done)->response->errcode() == pb::SUCCESS) {
                    write_raft_index(_region_id, _applied_index);
                }
            }
            break;
        }
        ...
        case pb::OP_SELECT_FOR_UPDATE: {
            _data_index = _applied_index;
            if (op_type == pb::OP_TRUNCATE_TABLE) {
                // olap region truncate时设置为active可以再次写入
            ...
            if (done != nullptr && ((DMLClosure*)done)->transaction != nullptr && (is_dml_op_type(op_type))) {
                ...
                auto res = ((DMLClosure*)done)->transaction->commit(_region_id, index);
                if (res.ok()) {
                    commit_succ = true;
                } else if (res.IsExpired()) {
            ...
            if (get_version() == 0 && res.errcode() != pb::SUCCESS && res.mysql_errcode() != ER_DUP_ENTRY) {
        ...
        case pb::OP_NONE: {
            ...
            if (FLAGS_enable_compute_storage_decoupled_mode && FLAGS_remote_compaction_send_no_op && _num_table_lines.load() > 0) {
                auto service = _rocksdb->get_remote_compaction_service();
                if (service != nullptr && service->enable_dsc(_table_id)) {
                    write_raft_index(_region_id, _applied_index);
                }
            }
        ...
        case pb::OP_ROLLUP_REGION_INIT: {
            _data_index = _applied_index;
            if (_rollup_region_init_index != -1) {
                DB_FATAL("rollup_region_init_index: %ld is not expect, OP_ROLLUP_REGION_INIT already exist", _rollup_region_init_index);
                if (done != nullptr) {
                    ((DMLClosure*)done)->response->set_errcode(pb::EXEC_FAIL);
                    ((DMLClosure*)done)->applied_index = _applied_index;
                }
            ...
            // 如果是OLAP_IMMUTABLE, 有可能会出现索引重复刷afs, 禁止在这个状态做索引
            if (_olap_state.load() == pb::OLAP_IMMUTABLE) {
                DB_WARNING("rollup_region_init_index: %ld, _olap_state is OLAP_IMMUTABLE, try again", _rollup_region_init_index);
                if (done != nullptr) {
                    ((DMLClosure*)done)->response->set_errcode(pb::EXEC_FAIL);
                    ((DMLClosure*)done)->applied_index = _applied_index;
                }
            ...
            _meta_writer->update_apply_index(_region_id, _applied_index, _data_index);
            _meta_writer->update_rollup_region_init_index(_region_id, _applied_index);
            _rollup_region_init_index = _applied_index;
            int64_t ddl_index_id = request.plan().nodes(0).derive_node().scan_node().ddl_index_id();
            int ret = _meta_writer->clear_rollup_batch_version(_region_id);
            if (ret != 0 && done != nullptr) {
                ((DMLClosure*)done)->response->set_errcode(pb::EXEC_FAIL);
                ((DMLClosure*)done)->applied_index = _applied_index;
                DB_FATAL("clear_rollup_batch_version fail op_type=%s, region_id: %ld, _applied_index:%ld, term:%ld, rollup_region_init_index: %ld",
                    pb::OpType_Name(request.op_type()).c_str(), _region_id, _applied_index, term, _rollup_region_init_index);
            } 
            ret = delete_hot_local_rocksdb(get_table_id(), ddl_index_id);
            if (ret != 0 && done != nullptr) {
                ((DMLClosure*)done)->response->set_errcode(pb::EXEC_FAIL);
                ((DMLClosure*)done)->applied_index = _applied_index;
                DB_FATAL("delete_hot_local_rocksdb fail op_type=%s, region_id: %ld, _applied_index:%ld, term:%ld, rollup_region_init_index: %ld",
                    pb::OpType_Name(request.op_type()).c_str(), _region_id, _applied_index, term, _rollup_region_init_index);
            } else if (done != nullptr) {
                ((DMLClosure*)done)->applied_index = _applied_index;
                ((DMLClosure*)done)->response->set_errcode(pb::SUCCESS);
            }
            DB_NOTICE("op_type=%s, region_id: %ld, _applied_index:%ld, term:%ld, rollup_region_init_index: %ld",
                      pb::OpType_Name(request.op_type()).c_str(), _region_id, _applied_index, term, _rollup_region_init_index);
            break;
        }
        case pb::OP_ROLLUP_REGION_FINISH: {
            _data_index = _applied_index;
            int ret = exec_rollup_region_finish_request(request, _rollup_region_init_index + 1, _applied_index, done, term, false);
            if (ret != 0) {
                DB_FATAL("exec_rollup_region_finish_request op_type=%s, region_id: %ld, _applied_index:%ld, term:%ld, rollup_region_init_index: %ld",
                      pb::OpType_Name(request.op_type()).c_str(), _region_id, _applied_index, term, _rollup_region_init_index);
                if (done != nullptr) {
                    ((DMLClosure*)done)->response->set_errcode(pb::EXEC_FAIL);
                    ((DMLClosure*)done)->applied_index = _applied_index;
        ...
        case pb::OP_ROLLUP_REGION_FAILED: {
            _data_index = _applied_index;
            int ret = exec_rollup_region_finish_request(request, _rollup_region_init_index + 1, _applied_index, done, term, true);
            if (ret != 0) {
                DB_FATAL("exec_rollup_region_finish_request op_type=%s, region_id: %ld, _applied_index:%ld, term:%ld, rollup_region_init_index: %ld",
                      pb::OpType_Name(request.op_type()).c_str(), _region_id, _applied_index, term, _rollup_region_init_index);
                if (done != nullptr) {
...
void Region::on_apply(braft::Iterator& iter) {
    for (; iter.valid(); iter.next()) {
        ...
        if (get_version() == 0
            ...
            if (done != nullptr &&
                (is_dml_op_type(request->op_type()) || is_2pc_op_type(request->op_type()))) {
                ((DMLClosure*)done)->apply_cost.reset();
            }
        ...
        for (auto& kv_op : request.kv_ops()) {
            ...
            if (get_version() != 0) {
                if (op_type == pb::OP_PUT_KV) {
                    ...
                    ret = txn->merge_kv(key.data(), kv_op.value());
                } else {
...
void Region::apply_kv_out_txn(const pb::StoreReq& request, braft::Closure* done, 
    ...
    auto resource = get_resource();
    int64_t num_increase_rows = 0;
    ScopeGuard auto_rollback([this, &txn, &commit_succ, &already_written, done]() {
        // rollback if not commit succ
        if (!commit_succ) {
            if (txn != nullptr) {
                txn->rollback();
    ...
    if (done == nullptr) {
        ...
        for (auto& kv_op : request.kv_ops()) {
            ...
            if (get_version() != 0) {
                if (op_type == pb::OP_PUT_KV) {     
                    ...
                    ret = txn->merge_kv(kv_op.key(), kv_op.value());
                } else {
        ...
        if (txn->use_cold_db() && txn->is_merge()) {
            // 冷数据在这里要写热的db
            txn = SmartTransaction(new Transaction(0, &_txn_pool));
            txn->set_resource(resource);
            txn->begin(Transaction::TxnOptions());
            txn->set_is_merge(true);
            txn->set_leader_merge_in_raft(FLAGS_leader_merge_in_raft);
        }
        if (txn->leader_merge_in_raft() && txn->is_merge()) {
            for (auto& kv_op : request.kv_ops()) {
                pb::OpType op_type = kv_op.op_type();
                if (op_type == pb::OP_MERGE_KV) {
                    int ret = txn->merge_kv(kv_op.key(), kv_op.value());
                    if (ret < 0) {
                        DB_FATAL("merge operation fail, op_type:%s, region_id: %ld, "
                            "applied_index: %ld, term:%ld", 
                            pb::OpType_Name(op_type).c_str(), _region_id, index, term);
                        return;
                    }
                } else {
                    // BUG merge操作中不会有其他op_type
                    DB_FATAL("other operation in merge, op_type:%s, region_id: %ld, "
                            "applied_index: %ld, term:%ld", 
                            pb::OpType_Name(op_type).c_str(), _region_id, index, term);
                    return;
                }
    ...
    uint64_t watt_stats_version = request.extra_req().watt_stats_version();
    // watt_stats新幂等机制
    if (watt_stats_version != 0) {
        auto s = _meta_writer->get_watt_stats_version(_region_id, watt_stats_version);
        if (s.IsNotFound()) {
            DB_DEBUG("write region_id: %ld watt_stats_version: %lu", _region_id, watt_stats_version);
            // 没有找到则可以写入，需要在txn中记录version
            if (_meta_writer->put_watt_stats_version(_region_id, watt_stats_version, txn) < 0) {
                return;
            }
        } else if (s.ok()) {
            // 此处不能使用txn，重复写入的情况下txn会rollback
            if (_meta_writer->put_skip_watt_stats_version(_region_id, index) < 0) {
                return;
            }
            DB_WARNING("repeat write region_id: %ld watt_stats_version: %lu, raft_index: %ld", _region_id, watt_stats_version, index);
            // 找到说明已经写入不可以再写
    ...
    auto res = txn->commit(_region_id, index);
    if (res.ok()) {
        commit_succ = true;
...
void Region::apply_txn_request(const pb::StoreReq& request, braft::Closure* done, int64_t index, int64_t term) {
    ...
    if (op_type != pb::OP_ROLLBACK || txn != nullptr) {
        if (op_type == pb::OP_PARTIAL_ROLLBACK) {
            std::set<int> need_rollback_seq;
            for (int rollback_seq : txn_info.need_rollback_seq()) {
                need_rollback_seq.insert(rollback_seq);
            }
...
int Region::generate_multi_split_keys() {
    ...
    // 检查主键第一个字段是整数型
    std::vector<pb::PrimitiveType> pk_fields;
    pk_fields.reserve(5);
    ...
    if (pri_info->fields[0].can_null) {
        DB_FATAL("table_id: %ld, region_id: %ld, first field in pk can be null", table_id, _region_id);
        return -1;
    }
    bool nullable_idx = pri_info->storage_type == pb::ST_NULL_KEY;
    // 拿起始userid
    TableKey tableKey(_split_param.split_key, true);
    std::string first_filed_str = tableKey.decode_start_key_string(pk_fields, 1, nullable_idx);
    ...
    for (uint32_t i = 0; i < _split_param.multi_new_regions.size(); ++i) {
        ...
        key.append_value(first_filed_value, nullable_idx);
...
void Region::start_split_for_tail(braft::Closure* done, int64_t applied_index, int64_t term) {    
    ...
    if (done != nullptr) {
        ...
        if (!iter->Valid()) {
            if (!iter->status().ok()) {
                DB_FATAL("Iterator error during tail split seek: %s, region_id: %ld, tableid:%ld",
                         iter->status().ToString().c_str(), _region_id, tableid);
            } else {
...
void Region::adjustkey_and_add_version_query(google::protobuf::RpcController* controller,
    ...
    if (validate_version(request, response, is_valid_version) != 0) {
        response->set_errcode(pb::INTERNAL_ERROR);
        response->set_errmsg("Fail to validate_version");
        DB_WARNING("Fail to validate_version, region_id: %ld, log_id:%lu",
                    _region_id, log_id);
        reset_region_status();
        return;
    }
...
void Region::validate_and_add_version(const pb::StoreReq& request, 
    ...
    // 分裂后的老region需要删除范围
    _need_vector_compact.store(true);
...
void Region::on_snapshot_save(braft::SnapshotWriter* writer, braft::Closure* done) {
    TimeCost time_cost;
    std::vector<std::string> faiss_files;
    _snapshot_path = writer->get_path();
    brpc::ClosureGuard done_guard(done);
    {
        BAIDU_SCOPED_LOCK(_reverse_index_map_lock);
        for (auto pair : _vector_index_map) {
            ...
            if (idx->is_separate()) {
                std::vector<std::string> faiss_name_vec;
                if (idx->write_to_separate_files(_snapshot_path, faiss_name_vec) != 0) {
                    done->status().set_error(EINVAL, "Fail to add snapshot");
                    DB_WARNING("Error while faiss write_to_separate_files, region_id: %ld, index_id: %ld", _region_id, index_id);
                    return;
                }
                faiss_files.insert(faiss_files.end(), faiss_name_vec.begin(), faiss_name_vec.end());
            } else {
                std::vector<std::string> faiss_name_vec;
                if (idx->write_to_file(_snapshot_path, faiss_name_vec) != 0) {
                    done->status().set_error(EINVAL, "Fail to add snapshot");
                    DB_WARNING("Error while faiss write_to_file, region_id: %ld, index_id: %ld", _region_id, index_id);
                    return;
                }
                faiss_files.insert(faiss_files.end(), faiss_name_vec.begin(), faiss_name_vec.end());
            }
            DB_WARNING("succ dump index:%s, index_id: %ld", _snapshot_path.c_str(), index_id);
        }
    }
    if (get_version() == 0) {
        wait_async_apply_log_queue_empty();
    ...
    for (auto& faiss_name : faiss_files) {
        if (writer->add_file(faiss_name) != 0) {
            done->status().set_error(EINVAL, "Fail to add snapshot");
            DB_WARNING("Error while adding extra_fs to writer, region_id: %ld", _region_id);
        }
    }
 
    _snapshot_path.resize(_snapshot_path.size() - 4); // 去掉temp
    char buf[50] = {0};
    snprintf(buf, sizeof(buf), "snapshot_%020ld", _applied_index);
    _snapshot_path += buf;
    DB_WARNING("region_id: %ld snapshot save complete, _snapshot_index:%ld _applied_index:%ld time_cost: %ld, _snapshot_path:%s",
                _region_id, _snapshot_index, _applied_index, time_cost.get_time(), _snapshot_path.c_str());
    reset_snapshot_status();

    done_guard.release();
    column_on_snapshot_save(writer, done);
}
...
void Region::snapshot(braft::Closure* done) {
    ...
    if (_rollup_region_init_index >= 0) {
        return;
    }
    bool need_no_op = false;
    ...
    SnapshotClosure* snapshot_done = dynamic_cast<SnapshotClosure*>(done);
    if (snapshot_done != nullptr) {
        snapshot_done->real_do_snapshot = true;
    }
    DB_WARNING("region_id: %ld do snapshot, snapshot_num_table_lines:%ld, num_table_lines:%ld "
            "snapshot_index:%ld, applied_index:%ld, rollup_region_init_index:%ld, snapshot_inteval_s:%ld",
            _region_id, _snapshot_num_table_lines, _num_table_lines.load(),
            _snapshot_index, _applied_index, _rollup_region_init_index, _snapshot_time_cost.get_time() / 1000 / 1000);
    done_guard.release();
    if (is_learner()) {
        _learner->snapshot(done);
    } else {
        if (FLAGS_enable_compute_storage_decoupled_mode && FLAGS_remote_compaction_send_no_op && need_no_op && is_leader() && _num_table_lines.load() > 0) {
            // 长时间没有写入触发no op时写入raft index，否则无法进行remote compaction
            auto service = _rocksdb->get_remote_compaction_service();
            if (service != nullptr && service->enable_dsc(_table_id)) {
                std::string address = Store::get_instance()->address();
                RpcSender::send_no_op_request(address, _region_id, _version, 1);
            }
        }
        _node.snapshot(done);
    }
...
void Region::on_snapshot_load_for_restart(braft::SnapshotReader* reader, 
    ...
    _done_applied_index = _applied_index;
    pb::OlapRegionInfo olap_info;
    _meta_writer->read_olap_info(_region_id, olap_info);
    _olap_state.store(olap_info.state());
    _rollup_region_init_index = _meta_writer->read_rollup_region_init_index(_region_id);
    ...
    DB_WARNING("success load snapshot, snapshot file not exist, "
                ...
                "applied_index:%ld data_index:%ld, rollup_region_init_index:%ld, raft snapshot index: %ld"
                " prepared_log_entrys_size: %ld, time_cost: %ld",
                _region_id, prepared_log_indexs.size(), 
                _applied_index, _data_index, _rollup_region_init_index, snapshot_index,
...
int Region::on_snapshot_load(braft::SnapshotReader* reader) {
    ...
    if (_restart && !Store::get_instance()->doing_snapshot_when_stop(_region_id)) {
        ...
        if (_is_binlog_region) {
            ...
            ret_binlog = restore_offline_binlog_info_on_snapshot_load(false);
            if (ret_binlog < 0) {
                DB_FATAL("binlog region_id: %ld ingest offline sst failed", _region_id);
                return -1;
            }
        }
        int ret = column_on_snapshot_load_restart();
        if (ret < 0) {
            DB_FATAL("region_id: %ld load column file failed", _region_id);
            return -1;
        }
        ...
        ON_SCOPE_EXIT(([this]() {
            if (is_addpeer()) {
                Concurrency::get_instance()->snapshot_load_concurrency.decrease_signal();
                if (_need_decrease) {
        ...
        if (_is_binlog_region) {
            ...
            ret_binlog = restore_offline_binlog_info_on_snapshot_load(true);
            if (ret_binlog < 0) {
                DB_FATAL("binlog region_id: %ld ingest offline sst failed", _region_id);
                return -1;
            }
        ...
        // ingest cold sst
        ret = ingest_cold_sst_on_snapshot_load();
        if (ret < 0) {
            DB_FATAL("olap region_id: %ld ingest cold sst failed", _region_id);
            return -1;
        }
        // ingest parquet file
        ret = column_on_snapshot_load(reader->get_path());
        if (ret < 0) {
            DB_FATAL("region_id: %ld ingest parquet file failed", _region_id);
            return -1;
        }
    ...
    _snapshot_path = reader->get_path();
    ...
    if (has_vector_index) {
        ...
        if (restore_faiss(_snapshot_path, files) != 0) {
            return -1;
        }
    }
    ...
    DB_WARNING("snapshot load success, region_id: %ld, num_table_lines: %ld,"
                " applied_index:%ld, data_index:%ld, rollup_region_init_index:%ld, region_info: %s, cost:%ld _restart:%d, _snapshot_path:%s",
                _region_id, _num_table_lines.load(), _applied_index, _data_index, _rollup_region_init_index,
                region_info.ShortDebugString().c_str(), time_cost.get_time(), _restart, _snapshot_path.c_str());
    // 分裂的时候不能做snapshot
...
int Region::ingest_snapshot_sst(const std::string& dir) {
    ...
    std::string remote_file = dir + "/" + SNAPSHOT_REMOTE_FILE;
    boost::filesystem::path remote_path(remote_file);
    if (boost::filesystem::exists(remote_path)) {
        std::ifstream remote_ifs(remote_file);
        std::string remote_string((std::istreambuf_iterator<char>(remote_ifs)),
            std::istreambuf_iterator<char>());
        std::vector<std::string> split_vec;
        boost::split(split_vec, remote_string, boost::is_any_of("\t"));
        auto s = _rocksdb->ingest_external_file_to_datacf(split_vec);
        if (!s.ok()) {
            DB_FATAL("ingest external file failed %s, remote %s files: %s", 
                s.ToString().c_str(), remote_file.c_str(), remote_string.c_str());
            return -1;
        }
        std::string local_store_ip = get_store_ip_port();
        for (auto& f : split_vec) {
            if (f.find(local_store_ip) == std::string::npos) {
                DB_FATAL("local ip : %s file name: %s", local_store_ip.c_str(), f.c_str());
            }
        }
        DB_WARNING("ingest external file success, remote %s files: %s", remote_file.c_str(), remote_string.c_str());
    }
...
void Region::vector_schema_change(std::unordered_map<int64_t, pb::IndexState>& vector_indexs) {
    ...
    for (auto& pair : cur_vector_index_map) {
        ...
        if (vec_idx == nullptr) {
            DB_FATAL("vec_idx is nullptr, region_id: %ld, index_id: %ld", _region_id, index_id);
            return;
        }
...
void Region::vector_truncate_del() {
    if (_shutdown) {
        return;
    }
    _multi_thread_cond.increase();
    ...
    for (auto& pair : vector_index_map) {
        ...
        if (vec_idx->truncate_del() != 0) {
            DB_WARNING("Fail to truncate_del, region_id: %ld, index_id: %ld", _region_id, index_id);
            return;
        }
        DB_WARNING("vector_index truncate_del success, region_id: %ld, index_id: %ld", _region_id, index_id);
    }
}

void Region::vector_compaction(bool is_manual) {
    ...
    need_compact = _need_vector_compact.load();
    _need_vector_compact.store(false);
    if (is_manual) {
        need_compact = true;
    }
    std::map<int64_t, VectorIndex*> vector_index_map;
    ...
    for (auto& pair : vector_index_map) {
        ...
        if (vec_idx->compact(resource->region_info, _num_table_lines.load(), need_compact) != 0) {
            DB_WARNING("Fail to compact, region_id: %ld, index_id: %ld", _region_id, index_id);
            return;
        }
        DB_WARNING("vector_index compaction success, region_id: %ld, index_id: %ld", _region_id, index_id);
    }
}
...
bool Region::has_sst_data(int64_t* seek_table_lines, int64_t* used_size) {
    ...
    std::unique_ptr<rocksdb::Iterator> iter(_rocksdb->new_iterator(read_options, _data_cf));
    if (seek_table_lines == nullptr || used_size == nullptr) {
        iter->Seek(table_prefix.data());
        return iter->Valid();
    } else {
        for (iter->Seek(table_prefix.data()); iter->Valid(); iter->Next()) {
            ++(*seek_table_lines);
            *used_size += iter->key().size() + iter->value().size();
        }
        if (!iter->status().ok()) {
            DB_FATAL("Iterator error in has_sst_data: %s, region_id: %ld",
...
bool Region::has_index_sst_data(int64_t index_id, int64_t* seek_table_lines) {
    MutTableKey table_prefix;
    table_prefix.append_i64(_region_id).append_i64(index_id);
    MutTableKey upper_bound;
    upper_bound.append_i64(_region_id).append_i64(index_id);
    upper_bound.append_u64(UINT64_MAX);
    upper_bound.append_u64(UINT64_MAX);
    upper_bound.append_u64(UINT64_MAX);
    ...
    if (seek_table_lines == nullptr) {
        ...
        if (!iter->status().ok()) {
            DB_FATAL("Iterator error in has_index_sst_data: %s, region_id: %ld",
                     iter->status().ToString().c_str(), _region_id);
        }
        return *seek_table_lines > 0;
    }
...
void Region::start_process_merge(const pb::RegionMergeResponse& merge_response) {
    ...
    int64_t used_size = 0;
    has_sst_data(&seek_table_lines, &used_size);
...
int Region::init_new_region_leader(int64_t new_region_id, std::string instance, bool tail_split) {
    ...
    // region_info->set_partition_num(get_partition_num()); // 当前系统未使用region partition_num，range分区不易维护
        ...
        for (auto& new_region : _split_param.multi_new_regions) {
            ...
            if (ret < 0) {
                ...
                for (auto& init_success_region : _split_param.multi_new_regions) {
                    ...
                    start_thread_to_remove_region(init_success_region.new_region_id, init_success_region.new_instance);
                }
                return;
            }
        }
...
void Region::write_local_rocksdb_for_split() {
    ...
    for (int64_t index_id : indices) {
        auto read_and_write = [this, &pk_info, &write_sst_lines, 
            ...
            os << FLAGS_db_path << "/" << "region_split_ingest_sst." << _region_id << "." 
                << _split_param.new_region_id << "." << index_id << "." << file_num++;
            ...
            for (iter->Seek(table_prefix.data()); iter->Valid(); iter->Next()) {
                ...
                num_write_lines++;
                if (writer->file_size() >= SST_FILE_LENGTH) {
                    s = writer->finish();
            ...
            if (!iter->status().ok()) {
                DB_FATAL("Iterator error during split region: %s, region_id: %ld, index: %ld",
            ...
            DB_WARNING("scan index:%ld, cost=%ld, file_size=%lu, file_num=%ld, lines=%ld, skip:%ld, region_id: %ld "
                    "level lines=[%ld,%ld,%ld]", 
                    index_id, cost.get_time(), file_size, file_num, num_write_lines, skip_write_lines, _region_id,
    ...
    if (table_info.engine == pb::ROCKSDB_CSTORE && !_is_global_index) {
        ...
        for (auto& field_info : table_info.fields) {
            ...
            auto read_and_write_column = [this, &pk_info, &write_sst_lines, end_key,
                ...
                if (!iter->status().ok()) {
                    DB_FATAL("Iterator error during column split: %s, region_id: %ld, field_id: %d",
                             iter->status().ToString().c_str(), _region_id, field_id);
                    _split_param.err_code = -1;
                    return;
                }
                s = writer->finish();
    ...
    if (has_vector_index) {
        ...
        try {
            ...
            DB_FATAL("remove error:%s, region_id: %ld", e.what(), _region_id);
            return; 
        }
        if (new_region->restore_faiss(_snapshot_path, files) != 0) {
            return;
        }
    }
...
int Region::restore_faiss(const std::string& path, const std::vector<std::string>& files) {
    BAIDU_SCOPED_LOCK(_reverse_index_map_lock);
    for (auto pair : _vector_index_map) {
        auto index_id = pair.first;
        std::string prefix = "/faissindex_" + std::to_string(index_id);
        std::string scalar_data_prefix = FAISS_SCALAR_DATA_PREFIX + std::to_string(index_id);
        std::string del_bitmap_prefix = FAISS_DEL_BITMAP_PREFIX + std::to_string(index_id);
        std::string not_cache_fields_prefix = FAISS_NOT_CACHE_FIELDS_PREFIX + std::to_string(index_id);
        auto idx = pair.second;
        if (idx == nullptr) {
            DB_FATAL("idx is nullptr, region_id: %ld, index_id: %ld", _region_id, index_id);
            return -1;
        }
        DB_WARNING("begin restore faiss, region_id: %ld index_id: %ld", _region_id, index_id);
        // 需要先载入未标量缓存的字段信息，再载入标量缓存数据
        for (auto& file : files) {
            if (boost::istarts_with(file, not_cache_fields_prefix)) {
                if (idx->read_not_cache_fields_from_file(path, file) != 0) {
                    DB_FATAL("Error while faiss read, region_id: %ld, index_id: %ld", _region_id, index_id);
                    return -1;
                }
            }
        }
        // 需要先载入标量缓存数据，再载入删除bitmap和向量索引
        for (auto& file : files) {
            if (boost::istarts_with(file, scalar_data_prefix)) {
                if (idx->read_scalar_data_from_file(path, file) != 0) {
                    DB_FATAL("Error while faiss read, region_id: %ld, index_id: %ld", _region_id, index_id);
                    return -1;
                }
            }
        ...
        for (auto& file : files) {
            if (boost::istarts_with(file, del_bitmap_prefix)) {
                if (idx->read_del_bitmap_from_file(path, file) != 0) {
                    DB_FATAL("Error while faiss read, region_id: %ld, index_id: %ld", _region_id, index_id);
                    return -1;
                }
            }
        ...
        if (idx->is_separate()) {
            for (auto& file : files) {
                if (boost::istarts_with(file, prefix)) {
                    if (idx->read_from_separate_file(path, file) != 0) {
                        DB_FATAL("Error while faiss read , region_id: %ld, index_id: %ld", _region_id, index_id);
                        return -1;
                    }
                }
            ...
            for (auto& file : files) {
                if (boost::istarts_with(file, prefix)) {
                    if (idx->read_from_file(path, file) != 0) {
                        DB_FATAL("Error while faiss read , region_id: %ld, index_id: %ld", _region_id, index_id);
                        return -1;
                    }
                    break;
        ...
        if (idx->restore_index(_region_info) != 0) {
            DB_FATAL("Error while faiss restore_index, region_id: %ld, index_id: %ld", _region_id, index_id);
            return -1;
        }
        DB_WARNING("restore faiss succ region_id: %ld, index_id: %ld", _region_id, index_id);
    }
    return 0;
}
        ...
        if (ret == 0) {
            if (_split_param.split_end_index == _applied_index) {
                ...
                if (seek_but_no_log_cost.get_time() > 500 * 1000LL) {
                    DB_FATAL("region: %ld split fail, seek log fail, split_end_index: %lu, _applied_index: %lu, seek_retry: %d", 
...
int Region::send_complete_to_one_new_region(const std::string& instance, const std::vector<std::string>& peers, int64_t new_region_id, 
    ...
    for (const std::string& addr : snapshot_instances) {
        pb::RegionIds snapshot_req;
        snapshot_req.add_region_ids(new_region_id);
        pb::StoreRes snapshot_res; 
...
void Region::print_log_entry(const int64_t start_index, const int64_t end_index) {
    ...
    int cnt = 0;
    auto process_single_log_entry = [region_id, &cnt](const rocksdb::Slice& rocksdb_key, const rocksdb::Slice& rocksdb_value, bool& need_break) {
        if (++cnt > 100) {
            need_break = true;
            return 0;
        }
        TableKey key(rocksdb_key);
        int64_t log_index = key.extract_i64(sizeof(int64_t) + 1);
        rocksdb::Slice value_slice(rocksdb_value);
        LogHead head(rocksdb_value);
        ...
        if (!store_req.ParseFromArray(value_slice.data(), value_slice.size())) {
            DB_FATAL("Fail to parse request fail, split fail, region_id: %ld", region_id);
            return 0;
        }
        DB_WARNING("region: %ld, log_index: %ld, term: %ld, req: %s",
                region_id, log_index, head.term, store_req.ShortDebugString().c_str());
        return 0;
    };
    RocksdbVars::get_instance()->raft_log_print_log_entry_count << 1;
    LogEntryReader::get_instance()->process_logs(region_id, start_index, end_index, process_single_log_entry);
}

int Region::exec_rollup_region_finish_request(const pb::StoreReq& request,
        const int64_t unappiled_begin_index, const int64_t unappiled_end_index, braft::Closure* done, int64_t term, bool is_failed) {
    if (_rollup_region_init_index == -1) {
    ...
    int64_t region_id = _region_id;
    auto process_single_log_entry = [region_id, term, this] (const rocksdb::Slice& key, const rocksdb::Slice& value, bool& /*need_break*/) {
        int64_t log_index = TableKey(key).extract_i64(sizeof(int64_t) + 1);
        rocksdb::Slice value_slice(value);
        LogHead head(value);
        if ((braft::EntryType)head.type != braft::ENTRY_TYPE_DATA) {
            DB_WARNING("log entry is not data, log_index:%ld, region_id: %ld, type: %d", log_index, region_id, (braft::EntryType)head.type);
            return 0;
        }
        value_slice.remove_prefix(MyRaftLogStorage::LOG_HEAD_SIZE);
        pb::StoreReq store_req;
        if (!store_req.ParseFromArray(value_slice.data(), value_slice.size())) {
            DB_FATAL("Fail to parse request fail, exec_rollup_region_finish_request fail, log_index:%ld, region_id: %ld, type: %d",
                    log_index, region_id, (braft::EntryType)head.type);
            return -1;
        }
        uint64_t txn_id = store_req.txn_infos_size() > 0 ? store_req.txn_infos(0).txn_id():0;

        pb::OpType op_type = store_req.op_type();

        // 做增量
        if (txn_id == 0 && op_type == pb::OP_KV_BATCH
            && !store_req.is_rollup_base() && store_req.kv_ops_size() > 0) {
            apply_kv_out_txn(store_req, nullptr, log_index, term, true);
        }
        return 0;
    };
    RocksdbVars::get_instance()->raft_log_rollup_region_count << 1;
    int ret = LogEntryReader::get_instance()->process_logs(region_id,
            unappiled_begin_index,
            unappiled_end_index - 1, // 传入的end是不包括的
            process_single_log_entry);
    if (ret != 0) {
        DB_WARNING("read unapplied raft log failed, region_id: %ld", region_id);
        return -1;
    }

    if (!is_failed && _olap_state.load() >= pb::OLAP_FLUSHED) {
        int64_t ddl_index_id = request.plan().nodes(0).derive_node().scan_node().ddl_index_id();
        pb::OlapRegionInfo olap_info;
        ...
        pb::OlapRegionIndexInfo* new_index_path = olap_info.mutable_new_olap_index_info();
        new_index_path->set_index_id(ddl_index_id);
        new_index_path->set_state(pb::OLAP_IMMUTABLE);
...
int Region::get_log_entry_for_split(const int64_t split_start_index, 
    ...
    int cnt = 0;
    int has_more_data = 0;
    auto process_single_log_entry = [&, this]
            (const rocksdb::Slice& rocksdb_key, const rocksdb::Slice& rocksdb_value, bool& need_break) {
        // 小batch发送
        if (++cnt > 10000) {
            // 主动break则表示还有数据
            need_break = true;
    ...
    RocksdbVars::get_instance()->raft_log_get_log_for_split_count << 1;
    int ret = LogEntryReader::get_instance()->process_logs(_region_id, split_start_index, -1, process_single_log_entry);
    if (ret != 0) {
        DB_WARNING("get log entry for split failed, region_id: %ld, start_index: %ld", _region_id, split_start_index);
        return -1;
    }
    if (batch_request.request_lens_size() > 0) {
        requests.emplace_back(batch_request);
...
int Region::get_split_key(const int64_t userid, std::string& split_key, int64_t& split_key_term) {
    int64_t tableid = get_global_index_id();
    if (tableid < 0) {
        DB_WARNING("invalid tableid: %ld, region_id: %ld", 
                    tableid, _region_id);
    ...
    SmartIndex pk_info = _factory->get_index_info_ptr(tableid);
    if (pk_info == nullptr || pk_info->fields.empty()) {
        DB_FATAL("region_id: %ld, table_id: %ld get index failed", _region_id, tableid);
        return -1;
    }
    pb::PrimitiveType first_key_type = pk_info->fields[0].type;
    ...
    if (first_key_type == pb::INT64) {
        start_seek_key.append_i64(userid);
    } else if (first_key_type == pb::UINT64) {
        start_seek_key.append_u64((uint64_t)userid);
    } else if (first_key_type == pb::INT32) {
        start_seek_key.append_i32((int32_t)userid);
    } else if (first_key_type == pb::UINT32) {
        start_seek_key.append_u32((uint32_t)userid);
    } else {
    ...
    for (iter->Seek(start_seek_key.data()); iter->Valid() 
        ...
        TableKey rocks_key(iter->key());
        if (rocks_key.size() <= 3 * sizeof(int64_t)) {
            DB_FATAL("region_id: %ld key size lt 24", _region_id);
            return -1;
        }

        user_key = iter->key().ToString();
        break;
    }

    if (!user_key.empty()) {
        split_key = user_key.substr(16);
        if (end_key_compare(split_key, end_key) < 0) {
            return 0;
        }
    }

    DB_FATAL("region_id: %ld userid: %ld get_split_key fail", _region_id, userid);
...
int Region::get_split_key(std::string& split_key, int64_t& split_key_term) {
    ...
    if (!iter->status().ok()) {
        DB_WARNING("Iterator error in get_split_key: %s, region_id: %ld",
                   iter->status().ToString().c_str(), _region_id);
        return -1;
    }
...
void Region::delete_local_rocksdb_for_ddl(int64_t table_id, int64_t index_id) {
    ...
    // 断开冷数据连接, 不会真正删afs
    status = _rocksdb->remove_cold_range(rocksdb::Slice(begin_key.data()), rocksdb::Slice(end_key.data()));
    if (!status.ok()) {
         DB_FATAL("DDL_LOG write remove cold ddl key failed, err_msg: %s, region_id: %ld, table_id:%ld index_id:%ld",
            status.ToString().c_str(), _region_id, table_id, index_id);
        return;
    }
    pb::OlapRegionInfo olap_info;
    std::set<std::string> need_delete_ext_path_set;
    int ret = _meta_writer->read_olap_info(_region_id, olap_info);
    if (ret < 0) {
        DB_FATAL("region_id: %ld read olap info failed", _region_id);
        return;
    }
    for (const auto& index_ext_path : olap_info.olap_index_info_list()) {
        if (index_ext_path.index_id() == index_id) {
            for (const auto& external_path: index_ext_path.external_path()) {
                need_delete_ext_path_set.insert(external_path);
            }
        }
    }
    for (auto olap_index_info_iter = olap_info.mutable_olap_index_info_list()->begin();
            olap_index_info_iter != olap_info.mutable_olap_index_info_list()->end();) {
        if (olap_index_info_iter->index_id() == index_id) {
            olap_index_info_iter = olap_info.mutable_olap_index_info_list()->erase(olap_index_info_iter);
        } else {
            ++olap_index_info_iter;
        }
    }
    if (need_delete_ext_path_set.size() > 0) {
        for (auto ext_path_iter = olap_info.mutable_external_full_path()->begin();
             ext_path_iter != olap_info.mutable_external_full_path()->end();) {
            if (need_delete_ext_path_set.count(*ext_path_iter) > 0) {
                ext_path_iter = olap_info.mutable_external_full_path()->erase(ext_path_iter);
            } else {
                ++ext_path_iter;
            }    
        }
...
int Region::delete_hot_local_rocksdb(int64_t table_id, int64_t index_id) {
    rocksdb::WriteOptions write_options;
    MutTableKey begin_key;
    MutTableKey end_key;
    begin_key.append_i64(_region_id).append_i64(index_id);
    end_key.append_i64(_region_id).append_i64(index_id).append_u64(UINT64_MAX);
    auto status = _rocksdb->remove_range(write_options, _data_cf, begin_key.data(), end_key.data(), true);
    if (!status.ok()) {
        DB_FATAL("DDL_LOG remove_index failed: code=%d, msg=%s, region_id: %ld table_id:%ld index_id:%ld", 
            status.code(), status.ToString().c_str(), _region_id, table_id, index_id);
        return -1;
...
int Region::update_ttl_info() {
    ...
    // 拒绝修改ttl字段的逻辑放在meta, 能到update_ttl_info 的修改都认为是合法的修改
    TTLInfo ttl_info = _factory->get_ttl_duration(get_table_id());
    if (ttl_info.ttl_duration_s <= 0) {
        return 0;
    }

    // 初始化ttl信息, 无ttl或者normal的表可以变更为field ttl
    if (!use_ttl() || use_normal_ttl()) {
        TTLFieldType new_type = TTLFieldType::T_NORMAL_TTL;
        std::shared_ptr<FieldInfo> ttl_field = ttl_info.ttl_field;
        if (ttl_field != nullptr) {
            // 只允许修改一次ttl字段
            ttl_info_updated = true;
            new_type = TTLFieldType::T_NON_TTL;
            ...
            for (const auto& field: value_fields) {
                if (field.id == ttl_field->id) {
                    new_type = global_index->id == global_index->pk
            ...
            for (const auto& field: global_index->fields) {
                if (field.id == ttl_field->id) {
                    new_type = TTLFieldType::T_FIELD_KEY;
                    break;
                }
            }

            if (new_type == TTLFieldType::T_NON_TTL) {
    ...
    if (ttl_info.ttl_duration_s > 0) {
        ...
        // online TTL
        if (!use_field_ttl()
                && ttl_info.online_ttl_expire_time_us > 0
                && ttl_info.online_ttl_expire_time_us != _online_ttl_base_expire_time_us) {
            ttl_info_updated = true;
            _online_ttl_base_expire_time_us = ttl_info.online_ttl_expire_time_us;
        }
    }
    std::string ttl_field_name = _ttl_field == nullptr ? "" : _ttl_field->short_name;
    if (ttl_info_updated && _init_success) {
        _txn_pool.update_ttl_info(use_ttl(), use_normal_ttl(), _online_ttl_base_expire_time_us);
        DB_WARNING("table_id: %ld, region_id: %ld, ttl_type: %d, ttl_field: %s, ttl_duration_s: %ld, online_ttl_expire_time_us: %ld, %s",
                get_table_id(), _region_id, static_cast<int32_t>(_ttl_type), ttl_field_name.c_str(), ttl_info.ttl_duration_s,
                ttl_info.online_ttl_expire_time_us, timestamp_to_str(ttl_info.online_ttl_expire_time_us / 1000000).c_str());
    }
    return 0;
}
...
int Region::get_ttl_timestamp(IndexInfo& index_info,
    ...
    rocksdb::Slice value_cpy = rocksdb_value;
    if (use_field_ttl()) {
        SmartRecord record = record_template->clone(true);
        int ret = 0;
        if (_ttl_type == TTLFieldType::T_FIELD_KEY) {
        ...
        } else if (_ttl_type == TTLFieldType::T_FIELD_PK_VALUE) {
            // main table的value内
            // 为空会有默认值
            ttl_decode(value_cpy, &index_info, _online_ttl_base_expire_time_us);
            TupleRecord tuple_record(value_cpy);
        ...
        } else if (_ttl_type == TTLFieldType::T_FIELD_GUI_VALUE) {
            // 全局唯一索引，且ttl字段在主键内不在索引内
            ttl_decode(value_cpy, &index_info, _online_ttl_base_expire_time_us);
            int pos = 0;
            ret = record->decode_primary_key(index_info, value_cpy, pos);
            if (ret != 0) {
                DB_WARNING("ttl decode field value from guidx value failed, index_id:%ld, field_id:%d", index_info.id, _ttl_field->id);
                return ret;
            }
        } else if (_ttl_type == TTLFieldType::T_FIELD_GI_KEY) {
    ...
    } else if (use_normal_ttl()) {
        ttl_timestamp = ttl_decode(value_cpy, &index_info, _online_ttl_base_expire_time_us);
    } else {
        DB_WARNING("not ttl region, should not be called. region_id: %ld", _region_id);
        return -1;
    }
    return 0;
}
...
void Region::ttl_remove_expired_data() {
    ...
    std::vector<int64_t> indices;
    bool delete_by_pk = use_field_ttl() && !_is_global_index; // 包含field ttl或者fulltext、vector index时，需要按照主键删除
    if (_is_global_index) {
        indices.push_back(global_index_id);
    } else {
        TableInfo table_info = _factory->get_table_info(main_table_id);
    ...
    for (int64_t index_id : indices) {
        ...
        if (!iter->status().ok()) {
            DB_FATAL("Iterator error during ttl delete: %s, region_id: %ld, index: %ld",
                     iter->status().ToString().c_str(), _region_id, index_id);
        }
        ...
        if (is_binlog_region()) {
            ...
            if (!iter->Valid()) {
                ...
                rocksdb::Slice key_slice(iter->key());
                key_slice.remove_prefix(2 * sizeof(int64_t));
                int64_t ts = decode_first_8bytes2int64(key_slice);
                int ret = _meta_writer->write_binlog_data_cf_oldest_ts(_region_id, ts);
                if (ret < 0) {
                    DB_FATAL("region_id: %ld, ts: %ld write binlog data cf oldest ts failed",_region_id, ts);
                }
...
void Region::check_peer_latency() {
    ...
    if (latency > FLAGS_check_peer_notice_delay_s * 1000 * 1000LL) {
        DB_WARNING("region %ld peer_latency: %ld, status: %s, dml_latency: %ld, commit_idx: %ld, apply_idx: %ld", 
                   _region_id, latency, state2str(status.state), dml_latency, status.committed_index, _applied_index);
    } 
...
int Region::append_pending_read(SmartFollowerReadCond c) {
    ...
    if (_wait_read_idx_queue == nullptr 
        ...
        c->set_failed();
        DB_FATAL("_wait_read_idx_queue append reads fail, region_id: %ld, log_id: %lu", _region_id, c->log_id);
        return -1;
    }
    return 0;
}
...
int Region::ask_leader_read_index(std::vector<SmartFollowerReadCond>& tasks) {
    ...
    ON_SCOPE_EXIT(([this, &success, &tasks]() {
        if (!success) {
            for (auto& req : tasks) {
                ...
                req->set_failed();
                DB_WARNING("region_id: %lu, log_id: %lu ask read index fail", _region_id, req->log_id);
            }
        }
    }));
    if (is_leader() && (!braft::FLAGS_raft_enable_leader_lease || _node.is_leader_lease_valid())) {
        ...
        for (auto i = 0; i <= 5; ++i) {
            ...
            if (res.errcode() == pb::NOT_LEADER && !res.leader().empty() && res.leader() != "0.0.0.0:0") {
    ...
    int64_t total_ask_leader_read_index_time = t.get_time();
    bool no_need_wait_read_idx = read_idx <= _done_applied_index;
    for (auto& req : tasks) {
        if (req == nullptr) {
            continue;
        }
        req->ask_leader_read_index_cost = total_ask_leader_read_index_time;
        req->read_index = read_idx;
        if (no_need_wait_read_idx) {
            req->finish_wait();
        } else {
            req->t.reset();
        }
    ...
    if (iter.is_queue_stopped()) {
        for (; iter; ++iter) {
            for (auto& req : (*iter).reqs) {
                ...
                req->wait_read_index_cost = req->t.get_time();
...
int Region::mpp_send_version_old(
    ...
    std::vector<std::shared_ptr<ExchangeSenderNode::Channel>> channels;
    ScopeGuard scope_guard([&channels]() {
        for (auto& channel : channels) {
            channel->close();
        }
    });
    for (int i = 0; i < pb_exchange_sender_node.receiver_destinations().size(); ++i) {
        std::shared_ptr<ExchangeSenderNode::Channel> channel = 
                std::make_shared<ExchangeSenderNode::Channel>(pb_exchange_sender_node.log_id(), 
                                                              pb_exchange_sender_node.fragment_instance_id(),
                                                              pb_exchange_sender_node.receiver_destinations(i),
                                                              true,
                                                              region_id,
                                                              false);
        if (channel == nullptr) {
            DB_WARNING("Fail to make channel");
            return -1;
        }
        if (channel->init() != 0) {
            DB_WARNING("Fail to init channel, %s", pb_exchange_sender_node.ShortDebugString().c_str());
            return -1;
        }
        channels.emplace_back(channel);
    }
    for (auto& channel : channels) {
        if (channel->send_version_old(is_merge, region_id, region_infos) != 0) {
            DB_WARNING("Fail to send_version_old");
            return -1;
        }
    }
    for (auto& channel : channels) {
...
void Region::add_peer_info(pb::StoreHeartBeatRequest& request, const pb::RegionInfo& region_info, 
                           int64_t applied_index, bool is_learner, bool has_exist_leader) {
    pb::PeerHeartBeat* peer_info = request.add_peer_infos();
    peer_info->set_table_id(region_info.table_id());
    int64_t main_table_id = region_info.table_id();
    if (region_info.has_main_table_id() && region_info.main_table_id() != 0 &&
            region_info.table_id() != region_info.main_table_id()) {
        main_table_id = region_info.main_table_id();
    }
    peer_info->set_main_table_id(main_table_id);
    peer_info->set_partition_id(region_info.partition_id());
    peer_info->set_region_id(region_info.region_id());
    peer_info->set_log_index(applied_index);
    peer_info->set_start_key(region_info.start_key());
    peer_info->set_end_key(region_info.end_key());
    peer_info->set_is_learner(is_learner);
    peer_info->set_exist_leader(has_exist_leader);
}

void NoOpTimer::run() {
    if (_region != nullptr && !_region->is_shutdown() && _region->is_leader() && !_region->is_splitting() && _region->get_version() > 0) {
        // 发no op
        std::string address = Store::get_instance()->address();

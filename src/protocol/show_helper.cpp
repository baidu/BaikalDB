...
namespace baikaldb {
DEFINE_int32(active_range_delay_days, 1, "active_range_delay_days, default:1");
...
void ShowHelper::init() {
    ...
    _calls[SQL_SHOW_ALL_TABLES] = std::bind(&ShowHelper::_show_all_tables,
            this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_BINLOG_INFO] = std::bind(&ShowHelper::_show_binlog_info, this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_INSTANCE_PARAM] = std::bind(&ShowHelper::_show_instance_param,
            this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_PLUGINS] = std::bind(&ShowHelper::_show_plugins,
            this, std::placeholders::_1, std::placeholders::_2);
    ...
    _calls[SQL_SHOW_META_BINLOG] = std::bind(&ShowHelper::_show_meta_binlog,
            this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_ACTIVE_RANGE] = std::bind(&ShowHelper::_show_active_range,
            this, std::placeholders::_1, std::placeholders::_2);
    ...
    _calls[SQL_SHOW_TRIGGERS] = std::bind(&ShowHelper::_show_triggers,
            this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_EVENTS] = std::bind(&ShowHelper::_show_triggers,
            this, std::placeholders::_1, std::placeholders::_2);
    ...
    _calls[SQL_SHOW_GRANTS] = std::bind(&ShowHelper::_show_grants,
            this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_ALL_PRIVILEGES] = std::bind(&ShowHelper::_show_all_privileges,
            this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_SWITCH_TABLES] = std::bind(&ShowHelper::_show_switch_tables,
            this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_CREATE_DATABASE] = std::bind(&ShowHelper::_show_create_database,
            this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_ALL_META] = std::bind(&ShowHelper::_show_all_meta,
            this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_MPP_SIGNS] = std::bind(&ShowHelper::_show_mpp_signs,
            this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_FETCHER_RESOURCE_TAG] = std::bind(&ShowHelper::_handle_client_query_template_dispatch,
            this, std::placeholders::_1, std::placeholders::_2);
    _calls[SQL_SHOW_INSULATE_FETCHER_RESOURCE_TAG] = std::bind(&ShowHelper::_handle_client_query_template_dispatch,
            this, std::placeholders::_1, std::placeholders::_2);
...
bool ShowHelper::execute(const SmartSocket& client) {
    ...
    if (key == "create") {
        if (split_vec.size() > 2 && (boost::iequals(split_vec[2], "table") || boost::iequals(split_vec[2], "view"))) {
            key = SQL_SHOW_CREATE_TABLE;
        } else if (split_vec.size() > 2 && boost::iequals(split_vec[2], "database")) {
            key = SQL_SHOW_CREATE_DATABASE;
        } else {
            client->state = STATE_ERROR;
            return true;
        }
...
bool ShowHelper::_show_events(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    if (client == nullptr || !client->user_info) {
        DB_FATAL("param invalid");
        //client->state = STATE_ERROR;
        return false;
...
bool ShowHelper::_show_create_table(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    ...
    for (auto& index_id : info.indices) {
        ...
        if (!compatible) {
            ...
            } else if (index_info.type == pb::I_PRIMARY || index_info.type == pb::I_FULLTEXT || index_info.type == pb::I_VECTOR) {
                oss << " " << index_map[index_info.type] << " ";
            } else if (index_info.type == pb::I_PRIMARY || index_info.type == pb::I_FULLTEXT || index_info.type == pb::I_VECTOR || index_info.type == pb::I_ROLLUP) {
                oss << " " << index_map[index_info.type] << " ";
            } else {
            ...
            if (index_info.type == pb::I_VECTOR) {
                oss << "\"vector_description\":\"" << index_info.vector_description << "\", ";
                oss << "\"dimension\":" << index_info.dimension << ", ";
                oss << "\"nprobe\":" << index_info.nprobe << ", ";
                oss << "\"metric_type\":\"" << pb::MetricType_Name(index_info.metric_type) << "\", ";
                std::string lower_vector_description = to_lower(index_info.vector_description);
                if (lower_vector_description.empty() || lower_vector_description.find("hnsw") != std::string::npos) {
                    oss << "\"efsearch\":" << index_info.efsearch << ", ";
                    oss << "\"efconstruction\":" << index_info.efconstruction << ", ";
                }
            }
    ...
    if (!compatible) {
        ...
        if (info.partition_info.has_primary_range_partition_type()) {
            oss << ", \"primary_range_partition_type\": \""
                << pb::RangePartitionType_Name(info.partition_info.primary_range_partition_type()) << "\"";
        }

        const size_t type_size = info.partition_info.gen_range_partition_types().size();
        if (type_size > 0) {
            oss << ", \"gen_range_partition_types\": [";
            for (size_t i = 0; i < type_size; ++i) {
                oss << "\"" << pb::RangePartitionType_Name(info.partition_info.gen_range_partition_types(i)) << "\"";
                if (i != type_size -1) {
                    oss << ",";
                }
            }
        ...
        if (info.partition_info.has_dynamic_partition_attr()) {
            const auto& dynamic_partition_attr = info.partition_info.dynamic_partition_attr();
            oss << ", \"dynamic_partition\": {";
            if (dynamic_partition_attr.has_enable()) {
                oss << "\"enable\":" << std::boolalpha << dynamic_partition_attr.enable();
            }
            if (dynamic_partition_attr.has_time_unit()) {
                oss << ",\"time_unit\":\"" << dynamic_partition_attr.time_unit() << "\"";
            }
            if (boost::algorithm::iequals(dynamic_partition_attr.time_unit(), "MONTH") &&
                                            dynamic_partition_attr.has_start_day_of_month()) {
                oss << ",\"start_day_of_month\":" << dynamic_partition_attr.start_day_of_month();
            }
            if (dynamic_partition_attr.has_start()) {
                oss << ",\"start\":" << dynamic_partition_attr.start();
            }
            if (dynamic_partition_attr.has_cold()) {
                oss << ",\"cold\":" << dynamic_partition_attr.cold();
            }
            if (dynamic_partition_attr.has_end()) {
                oss << ",\"end\":" << dynamic_partition_attr.end();
            }
            if (dynamic_partition_attr.has_prefix()) {
                oss << ",\"prefix\":\"" << dynamic_partition_attr.prefix() << "\"";
            }
            oss << "}";
        }
        if (info.engine == pb::DBLINK) {
            ...
            if (info.dblink_info.has_mysql_info()) {
                oss << ", \"mysql_info\": {";
                oss << "\"addr\":\"" << info.dblink_info.mysql_info().addr() << "\"";
                oss << ",\"username\":\"" << info.dblink_info.mysql_info().username() << "\"";
                // oss << ",\"password\":\"" << info.dblink_info.mysql_info().password() << "\""; // 隐去password
...
bool ShowHelper::_show_create_view(const SmartSocket& client,
    ...
    if (factory->get_table_id(full_name, table_id) != 0) {
        client->state = STATE_ERROR_REUSE;
        client->query_ctx->stat_info.error_code = ER_NO_SUCH_TABLE;
        client->query_ctx->stat_info.error_msg << "View '" << db << "."
                                               << table << "' not exist";
        return false;
    }
    ...
    oss << "CREATE VIEW `" << info.short_name << "` AS ";
    oss << info.view_select_stmt;
...
bool ShowHelper::_show_create_database(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    if (client == nullptr) {
        DB_FATAL("param invalid");
        return false;
    }
    ...
    do {
        ResultField field;
        field.name = "Create Database";
        field.type = MYSQL_TYPE_VARCHAR;
        field.length = 10240;
        fields.emplace_back(field);
    } while (0);

    std::string db = split_vec[3];

    std::string namespace_ = client->user_info->namespace_;
    if (db == "information_schema") {
        namespace_ = "INTERNAL";
    }
    std::string full_name = namespace_ + "." + db;
    int64_t db_id = -1;
    if (factory->get_db_id(full_name, db_id) != 0) {
        client->state = STATE_ERROR_REUSE;
        client->query_ctx->stat_info.error_code = ER_DBACCESS_DENIED_ERROR;
        client->query_ctx->stat_info.error_msg << "Database '" << db << "' not exist";
        return false;
    }
    ...
    std::ostringstream oss;
    oss << "CREATE DATABASE `" << db << "` ";
    if (info.has_charset) {
        oss << "DEFAULT CHARSET=" << pb::Charset_Name(info.charset) << " ";
    }
    std::ostringstream comment_oss;
    comment_oss << "COMMENT='{";
    if (info.has_engine) {
        comment_oss << "\"engine\":\"" << pb::Engine_Name(info.engine) << "\",";
    }
    if (!info.resource_tag.empty()) {
        comment_oss << "\"resource_tag\":\"" << info.resource_tag << "\"" << ",";
    }
    if (info.replica_num > 0) {
        comment_oss << "\"replica_num\":" << info.replica_num << ",";
    }
    if (info.byte_size_per_record > 0) {
        comment_oss << "\"byte_size_per_record\":" << info.byte_size_per_record << ",";
    }
    if (info.region_split_lines > 0) {
        comment_oss << "\"region_split_lines\":" << info.region_split_lines << ",";
    }
    if (info.learner_resource_tags.size() > 0) {
        comment_oss << "\"learner_resource_tag\":[";
        for (size_t i = 0; i < info.learner_resource_tags.size(); i++) {
            comment_oss << "\"" << info.learner_resource_tags[i] << "\"";
            if (i != info.learner_resource_tags.size() - 1) {
                comment_oss << ",";
            }
        }
    ...
    if (info.dists.size() > 0) {
        ...
        for (size_t i = 0; i < info.dists.size(); ++i) {
            comment_oss << "{";
            if (!info.dists[i].resource_tag.empty()) {
                comment_oss << "\"resource_tag\":\"" << info.dists[i].resource_tag << "\",";
            }
            if (!info.dists[i].logical_room.empty()) {
                comment_oss << "\"logical_room\":\"" << info.dists[i].logical_room << "\",";
            }
            if (!info.dists[i].physical_room.empty()) {
                comment_oss << "\"physical_room\":\"" << info.dists[i].physical_room << "\",";
            }
            comment_oss << "\"count\":" << info.dists[i].count << "}";
            if (i != info.dists.size() -1) {
                comment_oss << ",";
            }
        }
    ...
    if (!info.main_logical_room.empty()) {
        comment_oss << "\"main_logical_room\":\"" << info.main_logical_room << "\"" << ",";
    }
    if (info.binlog_infos.size() > 0) {
        comment_oss << "\"binlog_infos\":[";
        for (size_t i = 0; i < info.binlog_infos.size(); ++i) {
            comment_oss << "{";
            std::string binlog_infos_inner_str;
            if (info.binlog_infos[i].has_database()) {
                binlog_infos_inner_str += "\"database\":\"";
                binlog_infos_inner_str += info.binlog_infos[i].database();
                binlog_infos_inner_str += "\",";
            }
            if (info.binlog_infos[i].has_table_name()) {
                binlog_infos_inner_str += "\"table_name\":\"";
                binlog_infos_inner_str += info.binlog_infos[i].table_name();
                binlog_infos_inner_str += "\",";
            }
            if (info.binlog_infos[i].has_link_field() 
                    && info.binlog_infos[i].link_field().has_field_name()) {
                binlog_infos_inner_str += "\"link_field\":{\"field_name\":\"";
                binlog_infos_inner_str += info.binlog_infos[i].link_field().field_name();
                binlog_infos_inner_str += "\"},";
            }
            if (info.binlog_infos[i].has_partition_is_same_hint()) {
                binlog_infos_inner_str += "\"partition_is_same_hint\":";
                binlog_infos_inner_str += info.binlog_infos[i].partition_is_same_hint() ? "true" : "false";
                binlog_infos_inner_str += ",";
            }
            if (!binlog_infos_inner_str.empty()) {
                binlog_infos_inner_str.pop_back();
            }
            comment_oss << binlog_infos_inner_str << "}";
            if (i != info.binlog_infos.size() - 1) {
                comment_oss << ",";
            }
        }
        comment_oss << "]" << ",";
    ...
    if (!info.partition_info_str.empty()) {
        comment_oss << "\"partition_info_str\":\"" << info.partition_info_str << "\",";
    }
    std::string comment = comment_oss.str();
...
bool ShowHelper::_show_full_tables(const SmartSocket& client, const std::vector<std::string>& split_vec)  {
    ...
    for (uint32_t cnt = 0; cnt < tables.size(); ++cnt) {
        ...
        const std::string& table_full_table = namespace_ + "." + current_db + "."+ tables[cnt];
        SmartTable info = factory->get_table_info_ptr_by_name(table_full_table);
        if (info == nullptr) {
            DB_FATAL("table:%s not exist", table_full_table.c_str());
            continue;
        }
        std::vector<std::string> row;
        row.emplace_back(tables[cnt]);
        if (info->is_view) {
            row.emplace_back("VIEW");
        } else {
...
bool ShowHelper::_show_schema_conf(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    ...
    if (split_vec[2] == "pk_prefix_balance" 
            ...
            || split_vec[2] == "binlog_backup_days"
            || split_vec[2] == "use_column_storage"
...
bool ShowHelper::_show_binlog_info(const SmartSocket& client, const std::vector<std::string>& split_params) {
    SchemaFactory* factory = SchemaFactory::get_instance();
    if (factory == nullptr) {
    ...
    if (split_params.size() < 3) {
        ...
        if (split_params[2] == "detail") {
            ...
            if (split_params.size() > 4) {
                input_partition_id = strtoll(split_params[4].c_str(), NULL, 10);
            }
        } else {
            input_table_name = split_params[2];
...
bool ShowHelper::_show_all_tables(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    ...
    type_func_map["vector"] = [](const SmartTable& table) {
        return table != nullptr && table->has_vector_index;
    };
    ...
    for (auto& d_t_name : database_table) {
        ...
        std::vector<std::string> split_vec;
        boost::split(split_vec, d_t_name,
                     boost::is_any_of("\001"), boost::token_compress_on);
        if (split_vec.size() != 7) {
            DB_FATAL("database table name:%s", d_t_name.c_str());
            continue;
        }
    ...
    std::vector<std::string> names = {"namespace", "database_name", "table_name", "binlog_db",
                                      "binlog_table", "learner_tags", "dblink_info"};
    std::vector<ResultField> fields;
    fields.reserve(7);
    for (auto& name : names) {
        ResultField field;
...
bool ShowHelper::_show_virtual_index(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    ...
    for (auto& iter : sample.index_id_sample_sqls_map) {
        const int64_t meta_id = ::baikaldb::get_meta_id(iter.first);
        if (meta_id != 0) {
            continue;
        }
...
bool ShowHelper::_show_user(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    ...
    std::vector<std::string> names = {
            ...
            "resource_tag", "ddl_permission", "use_read_index", "enable_plan_cache", "request_range_partition_type", 
            "is_request_additional"};
    ...
    row.emplace_back(std::to_string(info->need_use_read_index()));
    row.emplace_back(std::to_string(info->enable_plan_cache));
    row.emplace_back(pb::RangePartitionType_Name(info->request_range_partition_type));
    row.emplace_back(std::to_string(info->is_request_additional));
    rows.emplace_back(row);
...
bool ShowHelper::_show_diff_region_size(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    ...
    for (auto& region_info : region_infos) {
        for (auto& ins : region_info.peers()) {
            instance_regions[ins].emplace_back(region_info.region_id());
        }
    }
...
bool ShowHelper::_show_plugins(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    if (client == nullptr) {
        DB_FATAL("param invalid");
        //client->state = STATE_ERROR;
        return false;
    ...
    // Make fields.
    std::vector<ResultField> fields;
    fields.emplace_back(make_result_field("Name", MYSQL_TYPE_VARCHAR, 1024));
    fields.emplace_back(make_result_field("Status", MYSQL_TYPE_VARCHAR, 1024));
    fields.emplace_back(make_result_field("Type", MYSQL_TYPE_VARCHAR, 1024));
    fields.emplace_back(make_result_field("Library", MYSQL_TYPE_VARCHAR, 1024));
    fields.emplace_back(make_result_field("License", MYSQL_TYPE_VARCHAR, 1024));
    ...
    do {
        std::vector<std::string> row;
        rows.push_back({"binlog", "ACTIVE", "STORAGE ENGINE", "NULL", "GPL"});
        rows.push_back({"InnoDB", "ACTIVE", "STORAGE ENGINE", "NULL", "GPL"});
    } while (0);
...
bool ShowHelper::_show_abnormal_switch(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    ...
    // fast_importer > 24h
    {
        pb::QueryRequest request;
        pb::QueryResponse response;
        request.set_op_type(pb::QUERY_FAST_IMPORTER_TABLES);
        MetaServerInteract::get_instance()->send_request("query", request, response);
...
void ShowHelper::_query_offline_binlog_info(int64_t table_id, int64_t region_id, 
    ...
    req.set_region_id(region_id);
    bool peer_has_diff = false;
    bool is_first_peer = true;
    int64_t region_oldest_ts = 0;
    int64_t region_newest_ts = 0;
    std::string peers;
    std::set<std::string> data_files;
    std::set<std::string> binlog_files;
    std::string data_paths;
    std::string binlog_paths;
    std::string invalid_paths;
    auto add_row = [&rows](int64_t region_id, const std::string& peers, int64_t oldest_ts, int64_t newest_ts, 
                           const std::string& data_paths, const std::string& binlog_paths, const std::string& invalid_paths,
                           bool peer_has_diff) {
        std::string oldest_ts_str = ts_to_datetime_str(oldest_ts);
        std::string newest_ts_str = ts_to_datetime_str(newest_ts);
        std::vector<std::string> row;
        row.reserve(8);
        row.emplace_back(std::to_string(region_id));
        row.emplace_back(peers);
        row.emplace_back("[ " + std::to_string(oldest_ts) + " - " + std::to_string(newest_ts) + " ]");
        row.emplace_back("[ " + oldest_ts_str + " - " + newest_ts_str + " ]");
        row.emplace_back(data_paths);
        row.emplace_back(binlog_paths);
        row.emplace_back(invalid_paths);
        row.emplace_back(peer_has_diff ? "has diff" : "");
        rows.emplace_back(row);
    };
    for (const auto& peer : info.peers()) {
        peers += peer + "; ";
        pb::StoreRes res;
        StoreInteract interact(peer);
        interact.send_request("query_binlog", req, res);
        int64_t peer_oldest_ts = res.extra_res().offline_binlog_info().oldest_ts();
        int64_t peer_newest_ts = res.extra_res().offline_binlog_info().newest_ts();
        std::string peer_data_paths;
        std::string peer_binlog_paths;
        std::string peer_invalid_paths;
        int data_path_num = 0;
        int binlog_path_num = 0;
        for (const auto& f : res.extra_res().offline_binlog_info().external_full_path()) {
            std::vector<std::string> vec;
            boost::split(vec, f, boost::is_any_of("."));
            if (vec.empty()) {
                invalid_paths += f + "\n";
            }
            if (vec.back() == "datasst") {
                ...
                if (is_first_peer) {
                    ...
                    if (data_files.count(f) == 0) {
                        peer_has_diff = true;
                    }
                }
            } else if (vec.back() == "binlogsst") {
                peer_binlog_paths += f + "\n";
                ++binlog_path_num;
                if (is_first_peer) {
                    binlog_files.insert(f);
                } else {
                    if (binlog_files.count(f) == 0) {
                        peer_has_diff = true;
                    }
                }
            } else {
        ...
        if (is_first_peer) {
            region_oldest_ts = peer_oldest_ts;
            region_newest_ts = peer_newest_ts;
        ...
        if (query_one_region) {
            invalid_paths.clear();
            add_row(region_id, peer, peer_oldest_ts, peer_newest_ts, peer_data_paths, peer_binlog_paths, invalid_paths, peer_has_diff);
            peer_has_diff = false;
        }
        is_first_peer = false;
    }
    if (!query_one_region) {
        add_row(region_id, peers, region_oldest_ts, region_newest_ts, data_paths, binlog_paths, invalid_paths, peer_has_diff);
    }
}

bool ShowHelper::_show_offline_binlog(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    // show offline_binlog tableid;
    // show offline_binlog tableid region_id;
    SchemaFactory* factory = SchemaFactory::get_instance();
    if (client == nullptr || client->query_ctx == nullptr || factory == nullptr) {
        DB_FATAL("param invalid");
        //client->state = STATE_ERROR;
    ...
    int64_t query_table_id = 0;
    int64_t query_region_id = 0;
    if (split_vec.size() == 4) {
        query_table_id = strtoll(split_vec[2].c_str(), NULL, 10);
        query_region_id = strtoll(split_vec[3].c_str(), NULL, 10);
    } else if (split_vec.size() == 3) {
        query_table_id = strtoll(split_vec[2].c_str(), NULL, 10);
    } else {
        client->state = STATE_ERROR;
    ...
    if (query_region_id == 0) {
        if (factory->get_all_partition_regions(query_table_id, &region_infos) != 0) {
            client->state = STATE_ERROR;
            return false;
        }
    } else {
        pb::RegionInfo info;
        if (factory->get_region_info(query_table_id, query_region_id, info) != 0) {
            client->state = STATE_ERROR;
            return false;
        }
        region_infos[query_region_id] = info;
    }
    
    // Make fields.
    std::vector<std::string> names = { "region_id", "peers", "offline ts", "offline datetime", "data files", "binlog files", "invalid files", "diff between peers"};
    std::vector<ResultField> fields;
    fields.reserve(8);
    for (auto& name : names) {
        ResultField field;
    ...
    for (const auto& region_info : region_infos) {
        int64_t region_id = region_info.first;
        const pb::RegionInfo& info = region_info.second;
        auto func = [this, &mutex, &rows, query_table_id, query_region_id, region_id, info, &cond]() {
            std::vector< std::vector<std::string> > region_rows;
            _query_offline_binlog_info(query_table_id, region_id, info, query_region_id != 0, region_rows);
            {
                BAIDU_SCOPED_LOCK(mutex);
                rows.insert(rows.end(), region_rows.begin(), region_rows.end());
            }
            cond.decrease_signal();
        };
        cond.increase_wait();
...
bool ShowHelper::_show_active_range(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    SchemaFactory* factory = SchemaFactory::get_instance();
    if (client == nullptr || client->query_ctx == nullptr || factory == nullptr) {
        DB_FATAL("param invalid");
        return false;
    ...
    // Make fields.
    std::vector<ResultField> fields;
    std::vector<std::string> names = {"lower_bound", "upper_bound"};
    fields.reserve(2);
    for(auto name : names) {
        ResultField field;
        field.name = name;
    ...
    std::string namespace_ = client->user_info->namespace_;
    int64_t tableid = -1;
    std::string table_full_name = namespace_ + "." + split_vec[2];
    if (0 != factory->get_table_id(table_full_name, tableid)) {
        DB_FATAL("get table_id failed, %s", table_full_name.c_str());
        return false;
    }
    auto table_ptr = factory->get_table_info_ptr(tableid);
    if (table_ptr == nullptr) {
        DB_FATAL("get_table failed, %s, %ld", table_full_name.c_str(), tableid);
        return false;
    }
    ...
    if (table_ptr->is_range_partition) {
        ...
        std::map<std::string, std::string> active_range;
        if (client->user_info->request_range_partition_type == pb::RPT_NEW_StatsEngine) {
            if (!table_ptr->partition_info.has_dynamic_partition_attr()) {
                DB_FATAL("table %s no dynamic_partition_attr", table_full_name.c_str());
                return false;
            }

            const auto& dynamic_partition_attr = table_ptr->partition_info.dynamic_partition_attr();
            if (!dynamic_partition_attr.has_time_unit() || !dynamic_partition_attr.has_cold()) {
                DB_FATAL("table %s no dynamic_partition_attr", table_full_name.c_str());
                return false;
            }
            ...
            const pb::PrimitiveType& partition_col_type = table_ptr->partition_info.field_info().mysql_type();
            TimeUnit unit;
            ...
            time_t cold_ts = current_ts + FLAGS_active_range_delay_days * 24 * 60 * 60LL;
            date_add_interval(cold_ts, cold, unit);
            std::string cold_str;
            if (partition_col_type == pb::DATE) {
                timestamp_to_format_str(cold_ts, "%Y-%m-%d", cold_str);
            } else {
                timestamp_to_format_str(cold_ts, "%Y-%m-%d %H:%M:%S", cold_str);
            }
        ...
        for (const auto& iter : active_range) {
            std::vector<std::string> row;
            row.reserve(2);
            row.emplace_back(iter.first);
            row.emplace_back(iter.second);
            rows.emplace_back(row);
        }
    } else {
        std::vector<std::string> row;
        row.reserve(2);
        row.emplace_back("0000-00-00");
        row.emplace_back("9999-99-99");
        rows.emplace_back(row);
    }
...
// show all_privileges dbs;
bool ShowHelper::_show_all_privileges(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    if (client == nullptr || client->user_info == nullptr || split_vec.size() < 3) {
        DB_FATAL("param invalid");
        return false;
    }
    std::unordered_set<std::string> filter_user_set = {"fengmai_user", "f1_user", "baikal_user", "root"};

    std::unordered_set<std::string> db_set;
    for (size_t i = 2; i < split_vec.size(); ++i) {
        db_set.emplace(split_vec[i]);
    }
    pb::QueryRequest request;
    pb::QueryResponse response;
    request.set_op_type(pb::QUERY_USERPRIVILEG);
    int ret = MetaServerInteract::get_instance()->send_request("query", request, response);
    DB_WARNING("req:%s res:%s", request.ShortDebugString().c_str(), response.ShortDebugString().c_str());
    ...
    // Make fields.
    std::vector<ResultField> fields;
    std::vector<std::string> names = {"user_name", "user_privilege"};
    for(auto name : names) {
        ResultField field;
        field.name = name;
        field.type = MYSQL_TYPE_STRING;
    ...
    // Make rows.
    std::vector< std::vector<std::string> > rows;
    for (const auto& user_privilege : response.user_privilege()) {
        if (filter_user_set.find(user_privilege.username()) != filter_user_set.end()) {
            continue;
        }
        if (user_privilege.namespace_name() != client->user_info->namespace_) {
            continue;
        }
        bool has_db = false;
        for (const auto& privilege_database : user_privilege.privilege_database()) {
            if (db_set.find(privilege_database.database()) != db_set.end()) {
                has_db = true;
                break;
            }
        }
        for (const auto& privilege_table : user_privilege.privilege_table()) {
            if (has_db) {
                break;
            }
            if (db_set.find(privilege_table.database()) != db_set.end()) {
                has_db = true;
                break;
            }
        }
        ...
        std::vector<std::string> row;
        row.emplace_back(user_privilege.username());
        row.emplace_back(pb2json(user_privilege));
        rows.emplace_back(row);
    }
    // Make mysql packet.
    if (_make_common_resultset_packet(client, fields, rows) != 0) {
...
bool ShowHelper::_show_switch_tables(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    if (client == nullptr) {
        DB_FATAL("param invalid");
        return false;
    }
    ...
    pb::QueryResponse response;
    request.set_op_type(pb::QUERY_USERPRIVILEG);
    request.set_user_name(split_vec[2]);
    int ret = MetaServerInteract::get_instance()->send_request("query", request, response);
    DB_WARNING("req:%s res:%s", request.ShortDebugString().c_str(), response.ShortDebugString().c_str());
    if (ret != 0 || response.errcode() != pb::SUCCESS) {
        DB_WARNING("send_request fail");
    ...
    std::vector<std::string> switch_table_vec;
    for (const auto& user_privilege : response.user_privilege()) {
        for (const auto& switch_table_id : user_privilege.switch_tables()) {
            SmartTable table_info = factory->get_table_info_ptr(switch_table_id);
            if (table_info == nullptr) {
                // 跳过被删除的表
                continue;
            }
            switch_table_vec.emplace_back(table_info->name);
        }
    }

    // Make fields.
    std::vector<ResultField> fields;
    std::vector<std::string> names = {"switch_tables"};
    for(auto name : names) {
        ResultField field;
        field.name = name;
        field.type = MYSQL_TYPE_VARCHAR;
    ...
    // Make rows.
    std::vector< std::vector<std::string> > rows;
    for (const auto& switch_table : switch_table_vec) {
        std::vector<std::string> row;
        row.emplace_back(switch_table);
        rows.emplace_back(row);
    }
...
bool ShowHelper::_show_mpp_signs(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    if (client == nullptr) {
        DB_FATAL("param invalid");
        return false;
    }
    ...
    if (0 != factory->get_mpp_signs_stat(signs, rows99, byte99, cnts, last_query_time)) {
        client->state = STATE_ERROR;
        return false;
    }
    ...
    // Make fields.
    std::vector<ResultField> fields;
    std::vector<std::string> names = {"signs", "db_handle_rows99", "db_handle_bytes99", "count", "last_query_time"};
    for(auto name : names) {
        ResultField field;
        field.name = name;
        field.type = MYSQL_TYPE_VARCHAR;
    ...
    for (int i = 0; i < signs.size(); ++i) {
        std::vector<std::string> row;
        row.emplace_back(std::to_string(signs[i]));
        row.emplace_back(std::to_string(rows99[i]));
        row.emplace_back(std::to_string(byte99[i]));
        row.emplace_back(std::to_string(cnts[i]));
        ExprValue query_time(pb::TIMESTAMP);
        query_time._u.uint32_val = last_query_time[i] / 1000000;
        query_time.cast_to(pb::DATETIME);
        row.emplace_back(query_time.get_string());
        rows.emplace_back(row);
    }
...
bool ShowHelper::_show_all_meta(const SmartSocket& client, const std::vector<std::string>& split_vec) {
   if (client == nullptr) {
        DB_FATAL("param invalid");
        return false;
    }
    ...
    std::unordered_map<int64_t, std::string> meta_id_name_map;
    if (factory->get_all_meta(meta_id_name_map) != 0) {
        DB_FATAL("get all meta name fail");
        client->state = STATE_ERROR;
        return false;
    }
    // Make fields.
    std::vector<ResultField> fields;
    std::vector<std::string> names = {"meta_id", "meta_name"};
    for(auto name : names) {
        ResultField field;
        field.name = name;
        field.type = MYSQL_TYPE_VARCHAR;
    ...
    // Make rows.
    std::vector<std::vector<std::string>> rows;
    for (const auto& kv : meta_id_name_map) {
        std::vector<std::string> row;
        row.emplace_back(std::to_string(kv.first));
...
bool ShowHelper::_handle_client_query_template_dispatch(const SmartSocket& client, const std::vector<std::string>& split_vec) {
    if (boost::iequals(split_vec[1], "meta")) {
        ...
        return _handle_client_query_template(client, "Disable Indexs", MYSQL_TYPE_VARCHAR,
                                      indexs);
    } else if (boost::iequals(split_vec[1], "fetcher_resource_tag")) {
        return _handle_client_query_template(client, "Fetcher_resource_tag",
                                             MYSQL_TYPE_VARCHAR, {FLAGS_fetcher_resource_tag});
    } else if (boost::iequals(split_vec[1], "insulate_fetcher_resource_tag")) {
        return _handle_client_query_template(client, "Insulate_fetcher_resource_tag",
                                             MYSQL_TYPE_VARCHAR, {FLAGS_insulate_fetcher_resource_tag});
    }
    return false;
}

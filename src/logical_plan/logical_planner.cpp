                ...
                for (int i = 0; i < ctx->plan.nodes_size(); i++) {
                    ...
                    if (table_index_map.count(table_id) > 0) {
                        for (auto& index_name: table_index_map[table_id]) {
                            ...
                            if (indexInfoPtr == nullptr || indexInfoPtr->state != pb::IS_PUBLIC ||
                                    indexInfoPtr->index_hint_status != pb::IHS_NORMAL) {
                                continue;
                            }
...
int LogicalPlanner::create_scala_func_expr(const parser::FuncExpr* item, 
    ...
    if (op == parser::FT_COMMON) {
        ...
        if (lower_fn_name == "last_value" && item->children.size() == 0) {
            ...
            node->set_num_children(0);
            node->mutable_derive_node()->set_string_val(_ctx->client_conn->last_value);
            return 0;
        }

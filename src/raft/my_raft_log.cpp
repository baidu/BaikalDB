...
struct MyRaftExtension {
    ...
    NewMyRaftLogStorage new_my_raft_log_storage;
    MyRaftLogStorage my_bin_log_storage;
    NewMyRaftLogStorage new_my_bin_log_storage;
...
static void register_once_or_die() {
    ...
    if (FLAGS_enable_new_raft_log_storage) {
        braft::log_storage_extension()->RegisterOrDie("myraftlog", &s_ext->new_my_raft_log_storage);
        braft::log_storage_extension()->RegisterOrDie("mybinlog", &s_ext->new_my_bin_log_storage);
    } else {

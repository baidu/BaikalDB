...
def worker3(start1, end):
    ...
    for i in range(int(start1),int(end)):
        sql="replace Baikaltest.studentPy(id,name1,name2,age1,age2,class1,class2,address,height) values(%d, 'baikal%d','baidu%d',%d, %d, %d, %d,'baidu%d',%d)"%(i,i,i,i,i,i,i,i,i)
...
def create_table():
    ...
    db = MySQLdb.connect(HOST, USER, PASSWORD, DATABASE, port=PORT)
    cursor = db.cursor()
    drop_table_if_exist = "drop table if exists Baikaltest.studentPy;"
    try:
        cursor.execute(drop_table_if_exist)
    except:
        pass
    sql = '''
    CREATE TABLE `Baikaltest`.`studentPy` (
        `id` int(10) NOT NULL ,
...
def check_rows_number():
    ...
    db = MySQLdb.connect(HOST, USER, PASSWORD, DATABASE, port=PORT)
    cursor = db.cursor()
    all_rows_sql = "select count(*) from Baikaltest.studentPy";
    class1_rows_sql = "select count(*) from Baikaltest.studentPy where class1>0";
    class2_rows_sql = "select count(*) from Baikaltest.studentPy where class2>0";
    age1_rows_sql = "select count(*) from Baikaltest.studentPy where age1>0";
    age2_rows_sql = "select count(*) from Baikaltest.studentPy where age2>0";
    ret = []
